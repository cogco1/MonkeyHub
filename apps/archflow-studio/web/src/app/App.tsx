...
export default function App({ server, initialDocumentIntent, initialRunId, task, active = true }: {
  ...
  const applyDirectModelAction = useCallback((action: DirectModelAction) => {
    if (!canDeleteModel || !deletableElementId) return;
    if ((action.kind === "pushPull" || action.kind === "move" || action.kind === "copy" || action.kind === "rotate") && action.target && action.target !== pushPullTarget) return;
    try {
      ...
      const captured = action.kind === "pushPull" ? { kind: "pushPull" as const, distance: action.distance, normal: normal! }
        : action.kind === "move" || action.kind === "copy" ? { kind: action.kind, translation: action.translation }
        : action.kind === "rotate" ? { kind: action.kind, angleDegrees: action.angleDegrees, axis: action.axis } : action;
