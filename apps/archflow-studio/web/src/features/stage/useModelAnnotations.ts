...
import { StudioApiError, TRANSPORT_ERROR, UNSUPPORTED_REQUEST, asStudioApiError, studio } from "../../api/client";
...
function createModel(scope: ModelAnnotationsOptions) {
...
export function createModelAnnotationsController() {
  ...
  return {
    ...
    model(options: ModelAnnotationsOptions) {
      ...
      if (!model) { model = createModel(options); models.set(key, model); }
      return model;
    },
  };
}
...
export function useModelAnnotations(
  ...
  const [localController] = useState(createModelAnnotationsController);
