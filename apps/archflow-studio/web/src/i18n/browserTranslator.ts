...
const ENGLISH_TO_CHINESE: TranslatorOptions = {
  sourceLanguage: "en",
  targetLanguage: "zh",
};
...
function protectTechnicalTokens(source: string): ProtectedSource {
  const tokens: string[] = [];
...
let translatorPreparation: Promise<BrowserTranslator | null> | null = null;
let automaticPreparationFailed = false;
let translationQueue: Promise<void> = Promise.resolve();

function installTranslator(created: BrowserTranslator): void {
...
async function createEnglishToChineseTranslator(): Promise<BrowserTranslator | null> {
...
function englishToChineseTranslator(): Promise<BrowserTranslator | null> {
 ...
 */
export async function prepareEnglishToChinese(): Promise<boolean> {
  try {
