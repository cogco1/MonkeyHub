...
async function drawingReady(readStart = 0) {
  ...
  assert.equal(await page.locator(".document-header > select").inputValue(), drawingSha);
...
try {
  await verifyProject();
  vite = await createServer({ root: webRoot, logLevel: "error",
  ...
  const deepUrl = new URL(uiOrigin);
  deepUrl.search = new URLSearchParams({ lang: "en", view: "documents", documentRun: oldEditingRun,
    documentSource: drawingSha, documentPage: "0" }).toString();
  await page.goto(deepUrl.href, { waitUntil: "domcontentloaded" });
  let savedA;
  let savedB;
  await step("the old browser editing preference and L6 deep link survive reading a group that already selects B", async () => {
    await drawingReady();
  ...
  await step("reloading the same browser restores B's full file and server ink while drawing storage stays unchanged", async () => {
    ...
    assert.equal(page.url(), deepUrl.href, "The original L6 deep link must remain the browser URL");
    await page.reload({ waitUntil: "domcontentloaded" });
    assert.equal(page.url(), deepUrl.href);
