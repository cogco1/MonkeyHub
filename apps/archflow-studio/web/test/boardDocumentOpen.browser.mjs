...
`, root, repoRoot], { cwd: apiRoot, encoding: "utf8", env: pythonEnv });
  ...
  await page.evaluate(() => { window.__sameDocument = "board"; });
  ...
  assert.equal(await page.getByRole("button", { name: "Edit this page", exact: true }).isEnabled(), true);
  ...
  assert.equal(opened.searchParams.get("view"), "documents");
  assert.equal(opened.searchParams.get("documentRun"), secondPage.runId);
  assert.equal(opened.searchParams.get("documentSource"), secondPage.assetSha256);
  assert.equal(opened.searchParams.get("documentPage"), "1");
  assert.equal(opened.searchParams.get("documentRevision"), secondPage.revisionRef);
  assert.equal(opened.searchParams.get("embedded"), "tool", "The host's embedding must survive the handoff");
  ...
  assert.equal(back.searchParams.get("view"), "board");
  assert.equal(back.searchParams.get("embedded"), "tool");
  for (const key of ["documentRun", "documentSource", "documentPage", "documentRevision"]) {
    assert.equal(back.searchParams.has(key), false, key);
  }
  assert.equal(await page.evaluate(() => window.__sameDocument), "board",
  ...
  // The same return without a host rail: the stage's own board entry comes back
  // to this board rather than opening a second tab beside it.
  await page.goto(`${origin}/?view=board&lang=en`, { waitUntil: "domcontentloaded" });
  await page.locator(".monkeyboard-initializing").waitFor({ state: "hidden" });
  await page.locator(".monkeyboard-canvas canvas").first().waitFor();
  await page.evaluate(() => { window.__sameDocument = "standalone-board"; });
  const standaloneBox = await page.locator(".monkeyboard-canvas").boundingBox();
  let placedPoint = null;
  for (const fraction of [0.3, 0.7, 0.5, 0.4, 0.6]) {
    placedPoint = [standaloneBox.x + standaloneBox.width * fraction, standaloneBox.y + standaloneBox.height / 2];
    await page.mouse.click(...placedPoint);
    if (await page.locator(".monkeyboard-context").isVisible()) break;
  }
  await page.locator(".monkeyboard-context").waitFor();
  const selectedPage = (await footer()).match(/board-pages\.pdf · (\d)\/2/);
  assert.ok(selectedPage, `A placed page was not selected: ${await footer()}`);
  await page.getByRole("button", { name: "Edit this page", exact: true }).click();
  await page.locator(".document-header").waitFor();
  assert.equal(await page.getByLabel("Page", { exact: true }).inputValue(), String(Number(selectedPage[1]) - 1));
  assert.equal(new URL(page.url()).searchParams.get("documentPage"), String(Number(selectedPage[1]) - 1));
  assert.equal(context.pages().length, 1, "The board entry must not open a second tab while a page is open here");
  await page.getByRole("button", { name: "MonkeyBoard · Board", exact: true }).click();
  await page.locator(".monkeyboard-canvas canvas").first().waitFor();
  assert.equal(context.pages().length, 1, "Returning stays in this tab");
  assert.equal(await page.evaluate(() => window.__sameDocument), "standalone-board");
  assert.equal(new URL(page.url()).searchParams.get("view"), "board");
  await page.locator(".monkeyboard-context").waitFor();
  assert.match(await footer(), new RegExp(`board-pages\\.pdf · ${selectedPage[1]}/2`));
  ...
  assert.equal(await page.locator(".document-header").count(), 0, "No page editor opens over an unsaved board");
  await page.locator(".monkeyboard-canvas canvas").first().waitFor();
  await boardAlert.waitFor();
  ...
  await page.mouse.dblclick(...placedPoint);
  await page.locator(".document-header").waitFor();
  const openedPage = new URL(page.url()).searchParams.get("documentPage");
  ...
  assert.equal(new URL(page.url()).searchParams.get("documentPage"), openedPage, "…on the page that was opened");
  assert.equal(await page.locator(".monkeyboard-canvas").count(), 0);
  ...
  // A second task can hold its own unsynced work, and returning would unmount it.
  // The board entry then keeps its existing separate tab instead.
  await page.mouse.click(...placedPoint);
  await page.locator(".monkeyboard-context").waitFor();
  await page.mouse.dblclick(...placedPoint);
  await page.locator(".document-header").waitFor();
  await page.getByRole("button", { name: "+ New task", exact: true }).click();
  await page.locator("[data-studio-task]:not([hidden]) .document-header, [data-studio-task]:not([hidden]) .stage").first().waitFor();
  assert.equal(await page.locator("[data-studio-task]").count(), 2, "The first task's view stays mounted beside the new one");
  const separateTab = await Promise.all([
    context.waitForEvent("page"),
    page.getByRole("button", { name: "MonkeyBoard · Board", exact: true }).click(),
  ]).then(([popup]) => popup);
  await separateTab.waitForURL((url) => url.searchParams.get("view") === "board", { timeout: 30_000 });
  assert.equal(new URL(separateTab.url()).searchParams.get("view"), "board", "The board is still reachable");
