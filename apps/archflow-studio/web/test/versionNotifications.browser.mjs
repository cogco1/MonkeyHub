...
const draft = "Keep this unsent design thought while a new version arrives.";
let phase = "setup";
let vite;
let browser;
let page;
...
try {
  ...
  await step("normal mode keeps one SSE connection with the developer drawer and event stream hidden", async () => {
    ...
    await until(() => composerInput().evaluate((input) => document.activeElement === input), Boolean,
      "Opening the conversation must focus its input");
  ...
  await step("default controls fit 1121 and 1440 pixel windows while the draft and editing choice survive resizing", async () => {
    ...
    for (const viewport of [{ width: 1121, height: 874 }, { width: 1440, height: 900 }]) {
      await setConversationOpen(false);
