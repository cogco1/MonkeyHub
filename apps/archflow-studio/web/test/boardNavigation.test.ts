...
test("the page address and the way back are the same tab", async (t) => {
  const { boardUrl, documentUrl } = await harness(t);
  const board = "http://127.0.0.1:8123/?view=board&embedded=tool&lang=en";
  const opened = documentUrl(board, source());
  const url = new URL(opened);
  assert.equal(url.searchParams.get("view"), "documents");
  assert.equal(url.searchParams.get("documentRun"), "drawing-run");
  assert.equal(url.searchParams.get("documentSource"), "a".repeat(64));
  assert.equal(url.searchParams.get("documentPage"), "1");
  assert.equal(url.searchParams.get("documentRevision"), source().revisionRef);
  assert.equal(url.searchParams.get("embedded"), "tool");
  const back = new URL(boardUrl(opened));
  ...
  assert.equal(boardUrl(opened), boardUrl(board));
});
