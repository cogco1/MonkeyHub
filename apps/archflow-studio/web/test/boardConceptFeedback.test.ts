...
test("one model-linked page plus an explicitly bound reference becomes one edit target and visual evidence", async (t) => {
  const geometry = await geometryModule(t);
  const target = element("image", "target", { width: 1000, height: 500 });
  const reference = element("image", "reference", { x: 1200, width: 600, height: 600 });
  const circle = element("ellipse", "zone", { x: 100, y: 100, width: 160, height: 120 });
...
async function handoffHarness(t: TestContext) {
  ...
  const snapshots = new Map<string, DocumentAnnotationsDto>([
    [targetDocument.runId, { projectId: "project", runId: targetDocument.runId, assetSha256: targetDocument.assetSha256,
      ...
      annotations: [] }],
    [referenceDocument.runId, { projectId: "project", runId: referenceDocument.runId, assetSha256: referenceDocument.assetSha256,
      pageIndex: 0, drawingRevisionRef: referenceDocument.revisionRef, revisionSha256: "reference-rev", comment: "",
      annotations: [] }],
  ]);
  studio.documents = async () => ({ projectId: "project", runId: null,
    documents: [structuredClone(targetDocument), structuredClone(referenceDocument)] });
  ...
  studio.documentFile = async (...args) => { calls.push({ method: "file", args });
    const runId = String(args[0]); const doc = runId === targetDocument.runId ? targetDocument : referenceDocument;
...
test("concept references travel as reference visuals while only the exact target page is saved and model-bound", async (t) => {
  ...
  const selection: BoardFeedbackSelection = {
    ...
    references: [{ image: {} as NonNullable<BoardFeedbackSelection["references"]>[number]["image"],
      document: structuredClone(referenceDocument), page: { ...referencePage }, source: { ...referenceSource }, referenceNote }],
  };
  ...
  assert.equal(request.documentVisuals[1].runId, referenceDocument.runId);
