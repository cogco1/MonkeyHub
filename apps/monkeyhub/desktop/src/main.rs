...
use monkeyarch_desktop::{is_status_url, DiagnosticLog, ExpectedIdentity, HealthError, LaunchConfig, OwnedRuntime, RuntimeLease, SOURCE_REVISION};
use serde::Serialize;
use std::{sync::{atomic::{AtomicBool, Ordering}, Arc, Mutex}, thread, time::{Duration, Instant}};
use tauri::{webview::NewWindowResponse, Manager, RunEvent, WebviewUrl, WebviewWindow, WebviewWindowBuilder, WindowEvent};
use uuid::Uuid;

#[derive(Clone, Serialize)]
struct Status { title: String, detail: String, log: String }
struct Shared {
    shutdown: AtomicBool,
    finished: AtomicBool,
...
fn paint_status(window: &WebviewWindow, shared: &Shared) {
    if window.url().is_ok_and(|url| is_status_url(&url)) {
        ...
        if let Ok(json) = serde_json::to_string(&status) {
            let _ = window.eval(&format!("window.renderDesktopStatus && window.renderDesktopStatus({json})"));
        }
    }
}

fn show_status(window: &WebviewWindow, shared: &Shared, log: &DiagnosticLog, state: &str, title: &str, detail: &str) {
    log.state(state, detail);
    *shared.identity.lock().unwrap() = None;
    *shared.status.lock().unwrap() = Status { title: title.into(), detail: detail.into(), log: log.path.display().to_string() };
    let _ = window.set_title(&format!("MonkeyArch · {title}"));
    if window.url().is_ok_and(|url| is_status_url(&url)) {
        paint_status(window, shared);
    } else {
        #[cfg(windows)] let url = "http://tauri.localhost/index.html";
        #[cfg(not(windows))] let url = "tauri://localhost/index.html";
        if let Err(error) = window.navigate(url.parse().unwrap()) { log.write(&format!("event=window-error detail={error}")); }
    }
}

fn supervise(window: WebviewWindow, config: LaunchConfig, shared: Arc<Shared>, log: DiagnosticLog, instance: Uuid) {
    show_status(&window, &shared, &log, "starting", "正在启动", "正在启动并核对本实例的本地运行时。");
    ...
    let mut runtime = match OwnedRuntime::spawn(&config, instance, log.clone()) {
        ...
        Err(error) => {
            ...
            shared.finished.store(true, Ordering::SeqCst);
            if shared.shutdown.load(Ordering::SeqCst) { window.app_handle().exit(0); }
            return;
        }
    };
    ...
    loop {
        ...
        if quitting && !stopping {
            stopping = true;
            show_status(&window, &shared, &log, "stopping", "正在关闭", "正在等待已接收的操作完成并关闭本实例。项目数据仍由原项目目录保存。");
            runtime.request_stop();
        }
        match runtime.try_wait() {
            Ok(Some(status)) => {
                log.write(&format!("event=exit pid={} code={status}", runtime.identity.process_id));
                if quitting {
                    log.state("stopped", "Owned Hub exited after shutdown");
                } else if !failed {
                    show_status(&window, &shared, &log, "failed", "运行时已退出", &format!("本地运行时意外退出（{status}）。请查看诊断日志，关闭应用后重新打开。"));
                }
                shared.finished.store(true, Ordering::SeqCst);
                if quitting || shared.shutdown.load(Ordering::SeqCst) { window.app_handle().exit(0); }
                return;
            }
            Err(error) if !failed => {
                failed = true;
        ...
        if !stopping && !failed && last_health.elapsed() >= Duration::from_millis(if ready { 1000 } else { 200 }) {
            last_health = Instant::now();
            match runtime.identity.check() {
                Ok(()) => {
                    if !ready || outage.is_some() {
                        if shared.shutdown.load(Ordering::SeqCst) { continue; }
                        *shared.identity.lock().unwrap() = Some(runtime.identity.clone());
                        if let Err(error) = window.navigate(config.url()) {
                            failed = true;
                            show_status(&window, &shared, &log, "failed", "无法打开工作界面", &error.to_string());
                            runtime.request_stop();
                        } else {
                ...
                Err(HealthError::Rejected(error)) => {
                    failed = true;
                    show_status(&window, &shared, &log, "failed", "运行时身份验证失败", &error);
                    runtime.request_stop();
                }
                Err(HealthError::Unavailable(error)) => {
                    if !ready && started.elapsed() >= config.startup_timeout {
                        ...
                        if since.elapsed() >= Duration::from_secs(3) && shared.identity.lock().unwrap().is_some() {
...
fn run() -> Result<(), String> {
    if std::env::args_os().skip(1).eq([std::ffi::OsString::from("--version")]) {
        println!("{}", serde_json::json!({"version": env!("CARGO_PKG_VERSION"), "sourceRevision": SOURCE_REVISION}));
        return Ok(());
    }
    let config = LaunchConfig::from_args(std::env::args_os().skip(1))?;
    let _lease = RuntimeLease::acquire(&config.runtime_root)?;
    ...
    std::fs::create_dir_all(&data_directory).map_err(|e| format!("Cannot prepare WebView cache: {e}"))?;
    let shared = Arc::new(Shared {
        shutdown: AtomicBool::new(false), finished: AtomicBool::new(false), identity: Mutex::new(None),
        status: Mutex::new(Status { title: "正在启动".into(), detail: "正在准备本地工作环境。".into(), log: log.path.display().to_string() }),
    });
    ...
    let app = tauri::Builder::default()
        .setup(move |app| {
            ...
            let window = WebviewWindowBuilder::new(app, "main", WebviewUrl::App("index.html".into()))
                ...
                .on_page_load(move |window, _| paint_status(window, &page_shared))
                .build()?;
            let close_shared = setup_shared.clone();
            window.on_window_event(move |event| {
                if let WindowEvent::CloseRequested { api, .. } = event {
                    if !close_shared.finished.load(Ordering::SeqCst) { api.prevent_close(); }
    ...
    app.run(move |_, event| {
        if let RunEvent::ExitRequested { api, .. } = event {
            if !shared.finished.load(Ordering::SeqCst) { api.prevent_exit(); }
...
fn main() {
    if let Err(error) = run() {
        ...
        #[cfg(windows)] unsafe {
            ...
            #[link(name = "user32")]
            extern "system" { fn MessageBoxW(hwnd: *mut std::ffi::c_void, text: *const u16, caption: *const u16, flags: u32) -> i32; }
            let message: Vec<u16> = error.encode_utf16().chain(Some(0)).collect();
            let title: Vec<u16> = "MonkeyArch · 启动失败".encode_utf16().chain(Some(0)).collect();
