...
import { projectStatus, workRows } from "./worktreeGraph";
...
type AppId = AppStatus["appId"] | "drawing" | "publish";
type Props = {
...
const tools: { id: AppId; label: "model" | "diagram" | "board" | "render" | "publish" | "fab" | "monitor"; icon: string; group: "workspace" | "system" }[] = [
...
export function ChatShell({ preferences, settings, settingsDirty = false, configuredProject, defaults, workspace, apps }: Props) {
  ...
  const rows = graph ? workRows(graph, projectRuntime?.operations ?? [], sessions, { you: t.workYou, unattributed: t.workUnattributed }) : [];
  ...
  const ensureProject = useCallback(async (target: string, projectId: string, appId?: AppId) => {
    ...
    try {
      ...
      const url = statuses.find((item) => item.appId === (!appId || (appId === "drawing" || appId === "publish") ? "monkeyarch" : appId) && item.state === "running")?.url;
  ...
  useEffect(() => {
    ...
    const id = query.get("view") === "board" ? "monkeyboard" : query.get("view") === "publish" ? "publish" : query.get("view") === "drawing" ? "drawing" : query.get("view") === "render" ? "monkeyrender" : "monkeyarch";
  ...
  useEffect(() => {
    ...
    if (panel && selectedTab?.id === "monkeymonitor") {
      ...
      url.searchParams.set("view", selectedTab.id === "monkeyboard" ? "board" : selectedTab.id === "publish" ? "publish" : selectedTab.id === "drawing" ? "drawing" : selectedTab.id === "monkeyrender" ? "render" : "arch");
    } else {
      url.searchParams.delete("runtimeId");
      if (["arch", "board", "drawing", "render", "publish", "monitor"].includes(url.searchParams.get("view") ?? "")) url.searchParams.delete("view");
    }
    window.history.replaceState(null, "", url);
    ...
    <nav className="chat-rail" aria-label={t.rail}>
      ...
      {projectInfo && <div className="chat-project-card" role="dialog" aria-label={t.projectInfo} onKeyDown={(event) => { if (event.key === "Escape") setProjectInfo(false); }}>
        ...
        {project ? <dl>
          ...
          {status && <><dt>{t.workCurrent}</dt><dd className="chat-project-card__head-line">{status.headLabel ?? project.stage ?? t.workUnaccepted}
            ...
            {worktreeError && !graph ? <span className="chat-muted" role="status">{t.workUnavailable}</span>
              ...
              : rows.map((row) => <div className="chat-project-card__line" key={row.key} data-kind={row.kind} data-reconcile={row.reconcile}>
                ...
                {row.kind !== "branch" && <small>{row.relation === "ahead" ? t.workAhead : row.relation === "behind" ? t.workBehind
                  : row.relation === "diverged" ? t.workDiverged : t.workSeparate}{row.kind === "result" && row.reconcile !== "none"
                  ? ` · ${row.reconcile === "can-combine" ? t.workCombine : row.reconcile === "conflict" ? "" : t.workReview}` : ""}</small>}
                {row.conflicts.length > 0 && <small className="chat-project-card__conflict" role="note">{t.workConflict(row.conflicts.join(", "))}</small>}
