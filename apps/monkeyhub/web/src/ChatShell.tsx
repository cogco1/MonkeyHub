...
import type { AppStatus, ApplicationSettingsDto, ChatCreateRequest, ChatDetail, ChatPostRequest, ChatProject, ChatProvider, ChatSummary, ChatWorkspace, HubError } from "./api/generated";
...
type Props = {
  ...
  onProjectBound: (settings: ApplicationSettingsDto) => void;
...
const words = {
  "zh-CN": {
    ...
    toolEmpty: "在这里查看项目", toolHint: "打开模型、图纸、画板或制作页面，聊天会一直保留。", projectBusy: "另一个项目仍在执行，请等它完成或停止后再切换工具。",
  ...
  en: {
    ...
    toolEmpty: "See the project here", toolHint: "Open the model, drawings, board or fabrication page while keeping your conversation.", projectBusy: "Another project is running. Wait for it to finish or stop it before switching tools.",
...
export function ChatShell({ preferences, settings, configuredProject, onProjectBound, defaults, workspace }: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [nextProjects, nextSessions, nextProviders, detail] = await Promise.all([
        request<ChatProject[]>("/api/chat/projects"), request<ChatSummary[]>("/api/chat/sessions"), request<ChatProvider[]>("/api/chat/providers"),
      ...
      setProjects(nextProjects); setSessions(nextSessions); setProviders(nextProviders);
  ...
  const selectChat = (item: ChatSummary) => {
    ...
    setProjectDir(item.projectDir); setChatId(item.id);
  };
  const selectProject = (item: ChatProject) => {
    const recent = sessions.find((session) => session.projectDir === item.projectDir);
  ...
  const ensureProject = async (target: string, appId: AppId = "monkeyarch") => {
    let saved = await request<ApplicationSettingsDto>("/api/settings/apps");
    if (saved.projectDir !== target) {
      const currentSessions = await request<ChatSummary[]>("/api/chat/sessions");
      if (currentSessions.some((item) => item.status === "running" && item.projectDir !== target)) throw new Error(t.projectBusy);
      const apps = await request<AppStatus[]>("/api/apps");
      const studio = apps.find((item) => item.appId === "monkeyarch");
      if (studio && ["running", "starting", "stopping"].includes(studio.state)) {
        await request("/api/apps/monkeyarch/stop", {});
        const deadline = Date.now() + 120000;
        while (true) {
          const states = await request<AppStatus[]>("/api/apps");
          const state = states.find((item) => item.appId === "monkeyarch");
          if (!state?.processId && state?.state !== "starting") break;
          if (Date.now() > deadline) throw new Error(t.projectBusy);
          await wait();
        }
      }
      saved = await request<ApplicationSettingsDto>("/api/settings/apps", { ...saved, projectDir: target, referenceRun: null }, "PUT");
      onProjectBound(saved); setTabs([]); setActiveTool(null);
    }
    let status = await request<AppStatus>(`/api/apps/${appId}/start`, {});
    const deadline = Date.now() + 60000;
    while (status.state !== "running" || !status.url) {
      if (status.state === "error" || status.state === "unavailable") throw new Error(status.error?.detail ?? status.state);
      if (Date.now() > deadline) throw new Error("The project service did not become ready.");
      await wait();
      status = (await request<AppStatus[]>("/api/apps")).find((item) => item.appId === appId)!;
    }
    const binding = await request<ApplicationSettingsDto>("/api/settings/apps");
    if (binding.projectDir !== target) throw new Error(t.projectBusy);
    return status.url;
  };

  const send = async (event?: FormEvent) => {
    event?.preventDefault();
    if (!projectDir || !draft.trim() || actionLock.current || running) return;
  ...
  const chooseModel = async (value: string | null) => {
    ...
    if (running || modelBusy || value === (chat.model ?? null)) return;
  ...
  const openTool = async (id: AppId, view?: Record<string, string>) => {
    if (!projectDir || actionLock.current) return;
    setPanel(true);
    const target = projectDir;
    actionLock.current = true; setToolBusy(id); setError(null);
    try {
      const location = await ensureProject(target, id);
      ...
      if (selection.current.projectDir !== target) return;
  ...
  return <div className="chat-shell" data-sidebar={sidebar} data-panel={panel} style={{ "--browser-width": `${panelWidth}px` } as CSSProperties}>
    <aside className="chat-sidebar" aria-label={t.projects}>
      ...
      <div className="chat-sidebar__body">
        <button className="chat-new" onClick={() => { setChatId(null); setChat(null); setError(null); input.current?.focus(); }}><Icon name="plus" /><span>{t.newChat}</span></button>
        ...
        <div className="chat-project-label"><span>{t.projects}</span><button className="chat-icon" aria-label={t.addExisting} title={t.addExisting} onClick={() => { setDialogError(null); addDialog.current?.showModal(); }}><Icon name="plus" /></button></div>
        {!projects.length && <p className="chat-muted chat-project-empty">{loading ? t.loading : t.emptyProjects}</p>}
        {projects.map((item) => <section className="chat-project" key={item.projectDir} data-selected={item.projectDir === projectDir}>
          ...
          {sessions.filter((session) => session.projectDir === item.projectDir).map((session) => <button key={session.id} className="chat-thread" aria-current={session.id === chatId ? "page" : undefined} onClick={() => selectChat(session)} title={session.title}>
    ...
    <main className="chat-main">
      ...
      <div className="chat-composer-wrap">
        {(error ?? (chat?.id === chatId ? chat?.error : null)) && <Failure failure={(error ?? chat!.error)!} language={preferences.language} labels={t} connection={connectionName}
          ...
          onChangeModel={running ? undefined : () => {
        ...
        {customModel !== null && <form className="chat-custom-model" onSubmit={(event) => { event.preventDefault(); const value = customModel.trim(); if (value) void chooseModel(value); else setCustomModel(null); }}>
    ...
    {panel && <><div className="chat-resizer" role="separator" aria-label={t.resize} aria-orientation="vertical" aria-valuemin={320} aria-valuemax={Math.max(320, window.innerWidth - 400)} aria-valuenow={panelWidth} tabIndex={0}
      ...
      <aside className="chat-browser" aria-label={t.browser}>
    ...
    <nav className="chat-rail" aria-label={t.rail}>
      ...
      {railGroups.map((group) => <div className="chat-rail__group" key={group.id} role="group" aria-label={t[group.caption]}>
        ...
        {tools.filter((item) => item.group === group.id).map((item) => <button key={item.id} className="chat-rail__tool" aria-pressed={panel && item.id === activeTool} disabled={!project || Boolean(toolBusy)}
          onClick={() => void openTool(item.id)}><Icon name={item.icon} /><span>{t[item.label]}</span></button>)}
      </div>)}
