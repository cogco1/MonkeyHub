...
import { appearanceFromSearch, applyAppearance, applicationUrl, DEFAULT_APPEARANCE, resolveAppearance, type AppearancePreferences, type Language } from "../../../shared-web/src/appearance.js";
...
const copy = {
  "zh-CN": {
    ...
    project: "项目目录", projectHelp: "MonkeyArch / MonkeyDiagram / MonkeyBoard 需要已有项目。MonkeyMonitor 和 MonkeyFab 无需项目。", projectPlaceholder: "已有项目的完整路径，可留空",
  ...
  en: {
    ...
    project: "Project directory", projectHelp: "MonkeyArch / MonkeyDiagram / MonkeyBoard need an existing project. MonkeyMonitor and MonkeyFab do not.", projectPlaceholder: "Full path to an existing project, optional",
...
function App() {
  ...
  const readSettings = useCallback(async () => {
    ...
    if (appearance.status === "fulfilled") {
      ...
      if (appearanceEdits.current === appearanceRevision) setPreferences(fabView ? appearanceFromSearch(window.location.search, resolved) : resolved); setAppearanceIssue(null);
  ...
  const descriptions: Record<AppId, CopyKey> = { monkeyarch: "arch", monkeydiagram: "diagram", monkeymonitor: "monitor", monkeyboard: "board", monkeyfab: "fab" };
  const fallback: AppStatus[] = (["monkeyarch", "monkeydiagram", "monkeymonitor", "monkeyboard", "monkeyfab"] as const).map((appId) => ({ appId, title: { monkeyarch: "MonkeyArch", monkeydiagram: "MonkeyDiagram", monkeymonitor: "MonkeyMonitor", monkeyboard: "MonkeyBoard", monkeyfab: "MonkeyFab" }[appId], serviceId: appId === "monkeyfab" ? "hub" : appId === "monkeymonitor" ? "monitor" : "studio", available: false, state: "stopped" }));
  return <><header className="toolbar"><strong className="wordmark">MonkeyHub</strong><span className={`connection ${connected ? "online" : ""}`} role="status">{connected ? t("connected") : statusIssue ? t("disconnected") : t("connecting")}</span><button className="btn" type="button" onClick={() => { void refreshApps(); if (savedLaunch === null || savedAppearance === null) void readSettings(); }}>{t("refresh")}</button><a className="btn" href="#settings">{t("settings")}</a></header>
    <main className="hub"><div className="section-heading"><h1>{t("apps")}</h1><span>{savedLaunch?.projectDir ?? t("noProject")}</span></div>
      ...
      <div className="apps">{(apps ?? fallback).map((app) => {
        ...
        return <article className="app-card" key={app.appId} data-app={app.appId}><div className="app-heading"><h2>{app.title}</h2><span className={`app-state state-${app.state}`}>{t(app.serviceId === "hub" && app.available ? "ready" : app.state)}</span></div><p>{t(descriptions[app.appId])}</p><div className="actions">{app.state === "running" && app.url ? <><a className="btn btn--primary" href={applicationUrl(app.url, preferences)} target="_blank" rel="noopener noreferrer">{t("open")}</a>{app.serviceId !== "hub" ? <button className="btn" type="button" disabled={!connected || pending} onClick={() => void act(app)}>{pending ? t("working") : t(app.serviceId === "studio" ? "stopShared" : "stop")}</button> : null}</> : <button className="btn" type="button" disabled={!connected || unavailable || !app.available || pending} onClick={() => void act(app)}>{unavailable ? t("unavailable") : pending ? t(app.state === "stopping" ? "stopping" : "starting") : t("start")}</button>}</div><ErrorMessage issue={actionIssues[app.appId] ?? app.error ?? null} language={preferences.language} /></article>;
