...
test("a refusal says what happened and the next step, with the server's words folded beneath (FN-4)", async (t) => {
  ...
  const cases: Array<[Refusal, RegExp, RegExp, RegExp]> = [
    ...
    [{ status: 409, code: "DOCUMENT_WRITE_FAILED", detail: "The source document could not be retained in its project." },
      /could not be saved in the project/, /project folder can be written to/, /没能保存到项目里/],
    [{ status: 409, code: "CANDIDATE_DELTA_INVALID", detail: "This candidate has competing retained changes." },
      /record saved in the project is inconsistent/, /Try another result or version/, /记录前后不一致/],
    [{ status: 409, code: "DOCUMENT_SOURCE_CONFLICT", detail: "The document source was replaced meanwhile." },
      /conflicts with the project/, /latest state/, /最新状态冲突/],
    [{ status: 422, code: "MODEL_ASSET_INVALID", detail: "The file is not a readable 3dm model." },
      /did not accept what this step sent/, /Check what you entered/, /没有接受这一步的内容/],
