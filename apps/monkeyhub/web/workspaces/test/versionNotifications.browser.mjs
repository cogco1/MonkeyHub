...
async function assertAnnotationNotePlacement() {
  const placement = await page.locator(".annotate__note").evaluateAll((notes) => {
    const controls = [...document.querySelectorAll(".stage-mode-switch, #stage-versions-panel .stage__versions-session, .stage-model .viewtools, .stage-decision, .project-bar")];
...
async function sessionState() {
  ...
  return {
    sourceMatch: await page.locator(".editing-base[data-source-match]").getAttribute("data-source-match"),
