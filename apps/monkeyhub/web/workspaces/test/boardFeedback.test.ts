...
test("returning a board arrow replaces its previous group and preserves other page work", async (t) => {
  ...
  const request = await h.prepareBoardDesignRequest(h.studio, chosen, "project-a", "  set height to 2.4  ");
...
test("a second send does not accumulate copies or overwrite an unrelated Diagram comment", async (t) => {
  ...
  await h.prepareBoardDesignRequest(h.studio, chosen, "project-a", "first");
  await h.prepareBoardDesignRequest(h.studio, chosen, "project-a", "second");
...
test("the reviewed selected-text message reaches intent once, with keeps and user edits preserved", async (t) => {
  const h = await harness(t), chosen = selection();
  const initial = await h.prepareBoardDesignRequest(h.studio, chosen, "project-a", chosen.selectedText);
  assert.equal(initial.utterance, chosen.selectedText, "Send without retyping uses the prefilled text once");
  const edited = "把入口向左移\n\n保留现有雨棚\n通道净宽不变";
  const request = await h.prepareBoardDesignRequest(h.studio, chosen, "project-a", edited);
  ...
  await assert.rejects(h.prepareBoardDesignRequest(h.studio, chosen, "project-a", "  "), (cause) => cause.code === "EMPTY_COMMENT");
...
test("missing, rebound, mismatched or resized drawing sources fail before annotation writes", async (t) => {
  for (const changed of [
    ...
    await assert.rejects(h.prepareBoardDesignRequest(h.studio, selection(), "project-a", "change"));
...
test("a page save conflict keeps the current page and never produces an intent handoff", async (t) => {
  ...
  await assert.rejects(h.prepareBoardDesignRequest(h.studio, selection(), "project-a", "change"), (cause) => cause === conflict);
