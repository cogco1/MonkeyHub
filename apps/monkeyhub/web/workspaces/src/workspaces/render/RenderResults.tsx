...
export function useDocumentImage(image: SourceDocumentDto | undefined, active: boolean, attempt = 0) {
  ...
  const key = image ? JSON.stringify([image.projectId, image.runId, image.assetSha256, image.revisionRef, image.fileName, attempt]) : "";
  const [loaded, setLoaded] = useState<{ key: string; url: string | null; error: string | null } | null>(null);
  useEffect(() => {
    if (!active) return;
    ...
    if (image) studio.documentFile(image.runId, image.assetSha256, image.fileName, image.revisionRef)
      .then(async (file) => {
        ...
        catch { throw new Error("The retained image could not be decoded."); }
        if (live) setLoaded({ key, url: objectUrl, error: null });
      })
      .catch((cause) => { if (live) setLoaded({ key, url: null, error: asStudioApiError(cause).detail }); });
    return () => { live = false; if (objectUrl) URL.revokeObjectURL(objectUrl); };
  }, [studio, key, active]);
  // A newly selected item must never display the previous item's bytes while reading.
  const current = loaded?.key === key ? loaded : null;
  return { url: current?.url ?? null, error: current?.error ?? null, loading: active && !!image && !current };
}
...
function ImagePane({ image, name, caption, zoom, reset, zh }: {
  image: ReturnType<typeof useDocumentImage>; name: string; caption: string; zoom: number; reset: string; zh: boolean;
}) {
  const host = useRef<HTMLDivElement>(null);
  useEffect(() => { host.current?.scrollTo(0, 0); }, [reset]);
  return <figure className="render-compare-pane">
    <figcaption>{caption}</figcaption>
    <div ref={host} className="render-image" tabIndex={0} aria-label={caption} aria-busy={image.loading}
      ...
      {image.url ? <img src={image.url} alt={name} draggable={false}
        style={{ width: zoom === 1 ? undefined : `${zoom * 100}%`, maxWidth: zoom === 1 ? "100%" : "none", maxHeight: zoom === 1 ? "100%" : "none" }} />
        : <p role="status">{image.loading ? (zh ? "读取图像中…" : "Loading image…") : (zh ? "图像暂不可用" : "Image unavailable")}</p>}
    </div>
  </figure>;
}
...
export default function RenderResults({ active, jobs, documents, selectedId, onSelect, onBoard, onReuse, onUpdateSource }: {
  ...
  useEffect(() => { if (!source) setView("result"); }, [source]);
  ...
  return <section className="render-results" aria-label={zh ? "渲染历史" : "Render history"}>
    <header className="render-toolbar">
      ...
      {(["result", "source", "compare"] as const).map((mode) => <button key={mode} type="button" disabled={!job || (mode !== "result" && !source)}
      ...
      <button type="button" onClick={() => { setZoom(1); setFit((value) => value + 1); }} disabled={!image && !source}>{zh ? "适应窗口" : "Fit"}</button>
      ...
      : <div className="render-content">
        ...
        <div className="render-detail">
          ...
          <div className="render-comparison" data-view={view}>
            {view !== "result" && <ImagePane image={sourceImage} name={source?.fileName ?? ""} caption={zh ? "本次生成的来源" : "Source for this generation"} zoom={zoom} reset={`${job?.jobId}:${fit}`} zh={zh} />}
            {view !== "source" && <ImagePane image={resultImage} name={image?.fileName ?? ""} caption={zh ? "渲染结果" : "Render result"} zoom={zoom} reset={`${job?.jobId}:${fit}`} zh={zh} />}
          </div>
          {job && <footer>
            ...
            <details><summary>{zh ? "来源与生成记录" : "Source & generation details"}</summary>
              <pre>{JSON.stringify({ source: job.request?.source ?? null, references: job.request?.references ?? [], modelSource: modelSource ?? null,
                sourceStageRef: source?.sourceStageRef ?? image?.sourceStageRef ?? null, sourceView: sourceView ?? null,
