...
import { ownerOf, projectStatus, workRows } from "../src/worktreeGraph.ts";
...
const graph: WorktreeGraphDto = {
  projectId: "p", revisionSha256: "r", warnings: [],
...
const words = { you: "You (Modeling)", unattributed: "Unattributed" };

test("the project status counts current, stale and background work without naming candidates", () => {
  assert.deepEqual(projectStatus(graph), {
    headLabel: null, accepted: false,
    drawings: { current: 1, stale: 0 }, renders: { current: 0, stale: 1, running: 1 },
    background: 2, separate: 2, conflicts: 1,
  });
});

test("owners come from the Hub journal: a conversation, the person in Modeling, or unattributed", () => {
  assert.equal(ownerOf("cand-4", operations, sessions, words), "Facade study");
  assert.equal(ownerOf("cand-5", operations, sessions, words), "You (Modeling)");
...
test("rows list running work first, conflicts before other results, and never the head itself", () => {
  ...
  assert.deepEqual(rows.map((row) => row.key), ["running:cand-6", "result:cand-5", "result:cand-4", "branch:facade-b"]);
