...
Completed local modeling gestures automatically update one project working draft. Reopening restores the exact source and local preview; editing alone does not create a candidate. Nothing has to be saved by hand: edits the working draft already holds never block **Restart to update**, because the restart keeps them and reopening their source restores them. Only an edit whose automatic save is still being written or failed holds the restart. Until Sync, both kinds keep a new chat context from starting from project state. **Sync** submits one frozen batch through the existing candidate workflow. The completed batch becomes the next saved editing base, and it also replaces the model on screen when nothing was edited or opened since Sync. Only the architect's own choices move the saved editing base: a completed Sync, **Continue from this version**, **Return to default editing base** or a version chosen in the panel; a Sync never overrides a base chosen meanwhile in another window. A candidate made by the agent (including options and program), an Arch proposal, combine, parameter locks, tracing or a Board sketch is shown automatically but never becomes the editing base on its own, including after reopening. The version panel shows the current draft, explicitly saved milestones and confirmed Stages, and lists every unlabelled generated candidate and Sync result among the automatic recovery points, newest first, however old it is. Saving a milestone does not accept a Stage or issue the project.
