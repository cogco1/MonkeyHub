...
MonkeyHub opens into project conversations: projects and chats on the left, conversation and composer in the center, and a resizable workspace on the right. The rail lists the project's surfaces (Modeling and Board, whose Layout mode is reached from a Board | Layout switch) and, apart from them, its Tools (Drawings, Render, Fabrication and Usage). Project workspaces and Drawings render directly in the same Hub frontend and retain local work when switching. Double-clicking a registered drawing page on Board opens its Diagram editor and returns to the same board. The installed Codex or Claude CLI continues each conversation using its native session. See the [shared entry and agent lookup sequence](../../docs/WORK_ENVIRONMENT_AND_EXTENSION_GUIDE.md#0-monkeyhub-统一入口).
...
The sidebar answers cross-project questions without opening projects one by one. **Tasks** lists the Agent work running or waiting in every open project — a conversation turn, a project operation or a candidate job still being built, each once and under the conversation that asked for it — with its project, what it is doing in plain words and whether it is running or waiting to start; choosing one opens that project and conversation. It is read from the runtime snapshot the Hub already receives and shows no ids. A project row carries a small "Running" badge while that project has such work. A "N new" schemes badge on the same row waits for the candidate admission records of #294; until the runtime reports them the row shows none rather than a guessed number. The footer shows **Usage**, the last seven days of model input and output tokens as MonkeyMonitor recorded them (read when the Hub reports Monitor running and again after a turn finishes; it opens the Usage tool), and **New version ready** when `/api/updates/status` reports a prepared update, which opens Settings at Software Update. Archived chats and Hub settings stay at the bottom.
...
Each of those calls is saved with the conversation as one record: the tool, its method and path, and the CLI's own word for how it ended. Both installed CLIs are read the same way — Codex reports one MCP item per call, Claude reports a `tool_use` block answered later by a `tool_result`, joined by its tool id — and a CLI's own file tools appear as the activity they are. Only this adapter's bound tools speak for the project: a result from anything else, or from a failed call, never names a candidate. A turn's calls fold into one row at the start of that turn, "Worked 1m 18s · 12 steps" ("用时 1分18秒 · 12 步"), measured from the turn's first to its last saved event; while the turn runs the row shows its clock and the step it is on in plain words, such as "Working · 0:42 · Read the board…". A refused or failed call is counted on the row ("1 failed") rather than disappearing, without opening it. Opened, the row lists the steps in plain words (reading the board, the design context or project documents, generating a scheme, looking through files, a web search); the lines the CLI reported and their diagnostics stay under that row's Technical details. The diagnostics are bounded — the values a person can act on, or a short preview with the remaining length stated; a whole state or schema document is never copied into the transcript. The Agent's text, attachments and documents, candidate actions, permission prompts and errors stay in the conversation, outside the fold. A row opened by hand stays open while that chat is shown; reopening the chat, or reloading, shows every turn folded again.

A conversation opens at its latest message and follows new output while the reader is at the end. A reader who scrolls up stays where they are while the Agent writes; a round ↓ button above the composer counts the new messages and scrolls back to the latest. Opening or folding a turn keeps it where it is on screen.
...
The rail has two groups. **Surfaces** are where the project is worked on: Modeling and Board. Layout (排版), the page layout and export of Publish, is Board's second mode: a Board | Layout switch at the top left of the surface moves between the two, both stay mounted with their work, the rail keeps Board pressed in either mode, and `view=publish` links still open Layout. **Tools** produce output over the project or report on the machine: Drawings, a 2D projection of the current project state rather than another architectural surface, Render, Fabrication, which needs no project, and Usage. Drawings opens inside the same project workspace, on the same Project Runtime and Working Head; it is not a separate project context. Pressing Drawings or Render again goes back to the surface it was opened over, with that view and any unrecorded edits as they were, or closes the panel when it was opened straight from the conversation (after a page reload, it only closes the panel). Where a drawing's source comes from and where it is saved are unchanged. Double-click a registered page on Board to edit it in Diagram and return to the same canvas. Diagram has no separate app entry.
...
Completed local modeling gestures automatically update one project working draft. Reopening restores the exact source and local preview; editing alone does not create a candidate. Nothing has to be saved by hand: edits the working draft already holds never block **Restart to update**, because the restart keeps them and reopening their source restores them. Only an edit whose automatic save is still being written or failed holds the restart. Until they are recorded, both kinds keep a new chat context from starting from project state. **Record** (记录, formerly Sync) submits one frozen batch through the existing candidate workflow. The completed batch becomes the next saved editing base, and it also replaces the model on screen when nothing was edited or opened since Record. Wherever unrecorded edits refuse an action (a base switch, the Design Tree's Continue, a Board note, Trace to 3D, a new chat context from project state, parameter locks), one click, **Record edits and continue**, records them and retries the action; if recording fails, the edits stay in the working draft. Only the architect's own choices move the saved editing base: a completed Record, **Continue from here**, **Accept as next Stage**, **Return to default editing base** or a version chosen in the panel; a Record never overrides a base chosen meanwhile in another window. Viewing never moves it: a Stage opened from Versions or the Design Tree is shown read-only, and a Board note made on another model version asks first, with **View only** as the default. An Arch proposal, combine, parameter locks, tracing or a Board sketch started in Modeling is shown there when it finishes, and like a candidate made by the agent (including options and program) it never becomes the editing base on its own, including after reopening. An Agent's result never switches the visible surface, opens the panel or replaces the model on screen: the conversation shows one Study card per request, and the Stage chip's “N options ready · View” notice opens the Design Tree on them. The version panel shows the current draft and explicitly saved milestones, and lists every unlabelled generated candidate and Record result among the automatic recovery points, newest first, however old it is. Saving a milestone does not accept a Stage or issue the project. Where the project has a Design Tree, it is the one entry to the design history: the panel's Stages, lines, explorations and candidates, and the footer's Stage count, viewing label and new badge, give way to one **Open in Design tree** link; the panel still sets the first Stage and lists the files. A runtime without a Design Tree keeps them in the panel.

P036 retains the working position and local recovery inside the project. It never deletes a run: a candidate made in Modeling or by the agent stays in the project whether or not a conversation, saved version or Stage refers to it. Hub periodically asks P036 to delete only superseded local recovery snapshots older than 24 hours: crash-recovery copies of unrecorded edits that a later autosave has replaced or cleared. Only the current snapshot is ever restored, and it is kept however old it is. Concurrent or failed saves report an error and preserve the previous recovery record.
...
Interjecting (#301): while a turn runs the composer stays open, and a message sent then goes to the same `POST /api/chat/sessions/{id}/messages`. It is kept as a user message marked `interjection` and shown at once; Stop stays beside it. Claude turns always read stream-json from stdin, so the message reaches the running CLI and the Agent reads it at its next step (`pending`, then `delivered` when the CLI echoes it back). Codex takes it into the live turn through the pinned adapter's `_session/steering` extension; an adapter without steering has its current step cancelled with `session/cancel`, and the turn continues with the message as the next prompt on the same ACP session (`restarted`), keeping everything already completed. A message that arrives as the turn ends becomes the next prompt; a stopped or failed turn leaves it `undelivered`. Interjections carry text only: files wait in the draft for the next turn.
