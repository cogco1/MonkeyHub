"""Read-only compatibility index for historical unscoped research records.

This module may read the pre-branch ``PrecedentQuery@1``,
``PrecedentAdoption@1``, ``P070DecisionCalibration@1``, and
``WebEvidenceSnapshot@1`` record families. It is not a canonical research
index, cannot close evidence obligations, and grants no new write authority.
"""
...
from archive.archflow.research.index import BasisIndexError


READ_ONLY_COMPAT = True
...
class BasisIndex:
    """Derived read-only shards for historical unscoped record families."""
    ...
    NEW_WRITE_AUTHORITY = NEW_WRITE_AUTHORITY
...
def decision_slug(decision_ref: str) -> str:
    """Filesystem-safe shard name for one historical decision ref."""
...
def build_basis_index(
    ...
    """Derive a read-only compatibility view from historical records."""
