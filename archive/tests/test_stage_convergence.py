...
import archive.archflow.control.convergence as canonical_convergence
import archive.archflow.state.stage_convergence as legacy_convergence
...
class StageConvergenceCompatibilityTests(unittest.TestCase):
    def test_legacy_state_facade_reexports_canonical_control_objects(self):
        self.assertEqual(
            canonical_convergence.__all__,
            legacy_convergence.__all__,
        )
        for name in canonical_convergence.__all__:
            with self.subTest(name=name):
                self.assertIs(
                    getattr(canonical_convergence, name),
                    getattr(legacy_convergence, name),
                )

    def test_canonical_helper_preserves_fixed_contract_digests(self):
        policy = StageConvergencePolicy(
            policy_id="digest-proof",
            stage="schematic-design",
            protected_refs=("fact:semantic:a",),
            mandatory_obligation_ids=("verify-a",),
        )
        state_evidence = StageConvergenceEvidence(
            state_digest="a" * 64,
            hard_gate_failure_refs=("gate-failure:a",),
        )
        self.assertEqual(
            "c3592f8906046f09a2397770db35af928765ece4388512e775d6457c667187a8",
            policy.policy_digest,
        )
        self.assertEqual(
            "a504e08034340236eaa5c5726e633477886981286cb80bdd12104ac4a5fb3096",
