...
for entry in (str(ROOT), str(ROOT / "tests"), str(ROOT / "tools")):
    if entry not in sys.path:
        sys.path.insert(0, str(entry))

from _probe_paths import WORKSPACE_PROJECTS, resolve_probe_root  # noqa: E402

import tests.integration.test_monument_derivation as M  # noqa: E402

from archflow.capabilities.basis_index import BranchDecisionContext  # noqa: E402
...
FLOOR_Y = 1.0            # project datum; the cross-space equality is design-authorized
...
# the current Stage-3 HOLD candidate envelope and column centres unchanged while
...
def _adoption_uri(pattern: str) -> str:
    ...
    for project_id in (PROJECT_ID, EVIDENCE_PROBE):
        try:
        ...
        if hits:
            hit = Path(hits[-1])
            return (
                f"project://{project_id}/runs/{hit.parents[1].name}"
                f"/records/{hit.name}"
            )
    raise SystemExit(f"no adoption record matches {pattern}")


def adoption_refs() -> dict[str, str]:
    return {
        "walls": _adoption_uri(
        ...
        "typology": _adoption_uri("precedent-adoption-????????*.json"),
...
def _pantheon_context():
    ...
    envelope = SiteBounds(minimum=(0, 0, 0), maximum=(57, 48, 78))
    ground = replace(
...
def _pantheon_proposal(context, *, option_id, radius):
        ...
        sorted(
            {
                ...
                *((x, z) for x in range(12, 46) for z in range(0, 23)),
            }
        )
    )
    levels = (
    ...
    volumes = (
        ...
        MassingVolume(
            "portico-volume",
            SiteBounds((12, 0, 0), (45, 27, 15)),
            ("portico",),
            evidence,
        ),
        MassingVolume(
            "transition-volume",
            SiteBounds((12, 0, 15), (45, 27, 27)),
            ("portico",),
            evidence,
        ),
        MassingVolume(
...
class _PantheonScriptedProvider(M._MonumentScriptedProvider):
    async def invoke(self, request):
        ...
        if schema == "SemanticSpatialAuthoringPrompt@1":
            ...
            proposal = _pantheon_geometry(state, stage=0)
            spatial_uri = (
...
def _pantheon_dome_sections(
    ...
    *,
    ...
    """Build dome rings from this project's axis, never a fixture global."""

    points: list[list[float]] = []
    for fraction in (0.0, 0.35, 0.6, 0.8, 0.93, 1.0):
        y = base_y + fraction * rise
        section_radius = (
            opening_radius
            if fraction == 1.0
            else max(
                radius * math.sqrt(1.0 - fraction * fraction),
                opening_radius,
            )
        )
...
def _pantheon_geometry(state, *, stage, prior=None):
    evidence = (
    ...
    operations = [
        ...
        M._loft(
            ...
            "dome-outer-object",
            "dome-binding",
            _pantheon_dome_sections(
                DRUM_OUTER,
                opening_radius=OCULUS_R,
                base_y=SPRING_Y,
                rise=23.5,
            ),
        ),
    ...
    if stage >= 3:
        ...
        for ring in range(COFFER_RINGS):
            for column in range(COFFER_COUNT):
                ...
                operations.append(
                    _coffer_cutter(
                        op_id, output, ring=ring, column=column
                    )
                )
    ...
    if prior is not None:
        cumulative_revised = {
            ...
            for component_id in M._STAGE_REVISED[prior_stage]
        }
        flagged_bindings = {
            binding_id
        ...
        revised_components = set(M._STAGE_REVISED[stage])
        revised_bindings = {
            binding_id
            for binding_id, (component_id, _) in bindings.items()
...
def stage_contracts(refs: dict[str, str]) -> dict[int, tuple]:
    ...
    stage0 = StageDeclarationContract(
        ...
        fields=(
            ...
            field("transition-height-m", DeclarationQuadrant.DIMENSIONS,
                  ...
                  (refs["junction"],) + uncertain,
                  statement=(
                      "height from site grade to the transition block top; "
                      "block carries the second pediment trace"
                  )),
    ...
    values0 = {
        ...
        "transition-height-m": TRANSITION_TOP_Y - GRADE_Y,
...
def _coffer_cutter(op_id, output, *, ring, column):
    ...
    surface = (
        CENTER_X + DRUM_INNER * normal[0],
        SPRING_Y + DRUM_INNER * normal[1],
        CENTER_Z + DRUM_INNER * normal[2],
    )
...
def _gated_persist_stage(repository, *, run, state, program, stage):
    contract, values = _CONTRACTS[stage]
    closure = None
    required_hard_refs = _hard_decision_refs(contract)
    if stage >= 1:
        if _STAGE_CLOSURE_RESOLVER is None:
            raise PantheonStageClosureError(
                "later-stage archival requires a configured P079/P080 "
                "closure resolver"
            )
        closure = _STAGE_CLOSURE_RESOLVER(
            stage=stage,
            run=run,
            design_state=state,
            contract=contract,
    ...
    _DECLARATION_LOG.append((stage, len(contract.fields)))
    return _ORIGINAL_PERSIST(
        repository, run=run, state=state, program=program, stage=stage
    )
...
def _detail_column_layout(program) -> tuple[dict[str, object], ...]:
...
def _detail_enrichment_plan(
    ...
    *,
    ...
    layout = _detail_column_layout(program)
    ...
    plan = {
        ...
        "massing_locks": {
            ...
            "drum_outer_radius_m": DRUM_OUTER,
            "drum_inner_radius_m": DRUM_INNER,
            "portico_width_m": PORTICO_W,
            "finished_floor_datum_relation": (
                "portico_finished_floor == main_entry_threshold == "
                "rotunda_finished_floor"
            ),
            "finished_floor_project_datum_m": FLOOR_Y,
        ...
        "detail_decisions": [
            ...
            },
            {
                ...
                "implementation": {
                    ...
                    "project_coordinate_datum_m": FLOOR_Y,
                    "absolute_historical_datum_ratified": False,
                },
                "dependencies": [
...
def _detail_rhino_overlay_script(
    ...
    *,
    ...
    plan_json = json.dumps(
        plan,
        # Rhino 8 RunPythonScript is IronPython in this environment.  Embed
        # non-ASCII text as JSON escapes so json.loads constructs Unicode
        # instead of a locale-decoded byte string (the Agrippa middle dots
        # otherwise fail while the detail measures are serialized).
        ensure_ascii=True,
        sort_keys=True,
        separators=(",", ":"),
    )
    inscription_literal = (
        "u'"
        + DETAIL_INSCRIPTION.encode("unicode_escape").decode("ascii")
        + "'"
    )
    header = "\n".join(
        (
            f"_detail_plan = json.loads({plan_json!r})",
            f"_detail_inscription = {inscription_literal}",
            "_detail_plan['detail_decisions'][1]['implementation']"
            "['text'] = _detail_inscription",
    ...
    body = textwrap.dedent(
        ...
        _detail_progress('proxy-layered')

        _detail_created = []
        _entablature_z = float(_detail_plan['massing_locks']['portico_entablature_base_y_m'])
        ...
        _detail_progress('column-instances-built')
        ...
        if _detail_asset_path:
            try:
                ...
                for item in _detail_plan['massing_locks']['column_centres']:
                    capital_z = item['base_y'] + 0.73 + 11.90
        ...
        for minimum, maximum, role in (
            ((11.45, -0.18, _entablature_z + 0.05), (44.55, 0.02, _entablature_z + 0.38), 'lower-architrave-fascia'),
            ((11.25, -0.28, _entablature_z + 1.88), (44.75, 0.02, _entablature_z + 2.12), 'frieze-crown-band'),
            ((10.95, -0.48, _entablature_z + 2.43), (45.05, 0.02, _entablature_z + 2.70), 'corona-band'),
        ):
        ...
        for index in range(47):
            ...
            guid = _detail_box((x - 0.16, -0.52, _entablature_z + 2.15), (x + 0.16, -0.08, _entablature_z + 2.42), cornice_layer)
        ...
        for side, midpoint, angle in (
            ('left', (20.125, -0.18, _entablature_z + 6.20), -slope),
            ('right', (35.875, -0.18, _entablature_z + 6.20), slope),
        ):
            guid = _detail_box(
        ...
        if inscription_id is None:
            raise Exception('inscription text object failed')
        _detail_created.append(inscription_id)
        _inscription_outlines = []
        _inscription_relief = []
        _detail_progress('inscription-text-created')

        door_layer = _detail_layers['door'][0]
        _door_leaves = []
        _door = _detail_plan['detail_decisions'][2]['implementation']
        _door_threshold_z = float(_door['threshold_y_m'])
        _door_head_z = float(_door['head_y_m'])
        for side, x0, x1 in (
            ('left', 25.775, 27.970),
            ('right', 28.030, 30.225),
        ):
            leaf = _detail_box((x0, 18.86, _door_threshold_z), (x1, 18.98, _door_head_z), door_layer)
            ...
            for panel in range(4):
                z0 = _door_threshold_z + 0.35 + panel * 1.70
        ...
        _detail_measures = {
            ...
            'primary_inscription_codepoints': [ord(character) for character in _detail_inscription],
            'primary_inscription_geometry_mode': _detail_plan['detail_decisions'][1]['implementation']['geometry_mode'],
            'primary_inscription_text_object_count': 1 if inscription_id else 0,
        ...
        _detail_progress('detail-measures-ready')
        '''
    ).strip()
    return header + "\n" + body + "\n"
...
def install(*, stage_closure_resolver=None, run_id: str = RUN_ID) -> None:
    """Parametrize the proven staging module with the reconstruction."""

    global _ORIGINAL_CONTEXT, _ORIGINAL_PERSIST, _CONTRACTS
    global _STAGE_CLOSURE_RESOLVER
    if _ORIGINAL_CONTEXT is None:
        _ORIGINAL_CONTEXT = M._monument_context
        _ORIGINAL_PERSIST = M._persist_stage
    if stage_closure_resolver is not None and not callable(
        stage_closure_resolver
    ):
        raise TypeError("stage_closure_resolver must be callable")
    _STAGE_CLOSURE_RESOLVER = stage_closure_resolver
    _CONTRACTS = stage_contracts(adoption_refs())
    M.PROJECT_ID = PROJECT_ID
    M.RUN_ID = run_id
    M.PROMPT = PROMPT
    M.CENTER_X = CENTER_X
    M.CENTER_Z = CENTER_Z
    M.DRUM_OUTER = DRUM_OUTER
    M.DRUM_INNER = DRUM_INNER
    M._STAGE_COMPONENTS = {
        1: (
            (
            ...
            (
                ...
                "Realize the current five-step axial-approach candidate; "
                "the historical count remains unresolved.",
            ),
            (
    ...
    original_findings = M._stage_symmetry_findings

    def _findings_with_transition(scene_objects, stage):
        from archflow.evaluation import axial_group_offsets

        findings = list(original_findings(scene_objects, stage))
        findings += list(
            axial_group_offsets(
                scene_objects,
                axis_value=CENTER_X,
    ...
    M._stage_symmetry_findings = _findings_with_transition
    M._monument_context = _pantheon_context
    M._monument_proposal = _pantheon_proposal
    M._monument_geometry = _pantheon_geometry
    M._MonumentScriptedProvider = _PantheonScriptedProvider
    M._persist_stage = _gated_persist_stage
...
def _agent_evidence_proposal() -> dict[str, object]:
...
def _run_hold_candidate(
    ...
    *,
    ...
    evidence_ref = repository.put_json(
        ...
        payload=_agent_evidence_proposal(),
    )
    authorization_ref = repository.put_json(
        ...
        payload={
            ...
            "project_id": run.project_id,
            "run_id": run.run_id,
            "request_text": "那你用新的这版，重新推导万神殿到stage4",
    ...
    datum_authorization_ref = repository.put_json(
        ...
        payload={
            ...
            "run_id": run.run_id,
            "request_context": [
                "大门好像和前面台阶的标高不对，你检查一下，然后看看为啥",
                "stage4我们正在深化，然后我判定可以了你在做stage5",
                "嗯嗯，弄吧",
            ],
    ...
    context = M._rebase_context(M._monument_context(), run)
    ...
    provider = M._MonumentScriptedProvider(context)
    ...
    for stage in range(4):
        if stage == 0:
            ...
            current_state = M._next_state(predecessor_state, stage=stage)
            lifecycle = M.compile_semantic_geometry_lifecycle(
                ...
                geometry_proposal=M._monument_geometry(
                    current_state,
                    stage=stage,
        ...
        realized = _realized_declaration_values(current_program, stage)
        contract, _ = _CONTRACTS[stage]
        ...
        review_ref = repository.put_json(
            ...
            payload={
                ...
                "open_conflicts": (
                    []
                    if stage < 1
                    else [front_step_conflict_ref.uri]
                ),
                "evidence_proposal_ref": evidence_ref.uri,
...
def _write_speculative_rhino_workspace(
    ...
    translation = translate_to_rhino_python(
        program,
        provenance={
            "coordinate_transform": _PANTHEON_COORDINATE_TRANSFORM[
                "mapping"
            ],
            "program_digest": program.program_digest,
            "rhino_vertical_axis": _PANTHEON_COORDINATE_TRANSFORM[
                "rhino_vertical_axis"
            ],
            "source_vertical_axis": _PANTHEON_COORDINATE_TRANSFORM[
    ...
    wrapper = "\n".join(
        (
            ...
            *_rhino_metric_document_setup_lines(),
            ...
            f"'source_vertical_axis': {_PANTHEON_COORDINATE_TRANSFORM['source_vertical_axis']!r}, "
            f"'rhino_vertical_axis': {_PANTHEON_COORDINATE_TRANSFORM['rhino_vertical_axis']!r}, "
            f"'coordinate_transform': {_PANTHEON_COORDINATE_TRANSFORM['mapping']!r}}}",
...
def _write_speculative_detail_rhino_workspace(
    ...
    *,
    ...
    progress_path = workspace / "cad-detail-progress.json"
    escaped = {
        "model": str(model_path),
        "measures": str(measures_path),
        "semantics": str(semantics_path),
    ...
    wrapper = "\n".join(
        (
            ...
            "'status': 'running'}",
            f"_detail_progress_path = {escaped['progress']!r}",
            "_detail_progress_state = {'schema': "
            "'P069RhinoDetailProgress@1', 'status': 'running', "
            "'last_completed': 'wrapper-started', 'events': []}",
            "def _detail_progress(marker):",
            "    _event = {'marker': marker, 'time_unix': time.time()}",
            "    _detail_progress_state['last_completed'] = marker",
            "    _detail_progress_state['events'].append(_event)",
            "    with open(_detail_progress_path, 'w') as _f:",
            "        json.dump(_detail_progress_state, _f, sort_keys=True, "
            "ensure_ascii=True)",
            "_detail_progress('wrapper-started')",
            "try:",
            *_rhino_metric_document_setup_lines(),
            textwrap.indent(translation.script, "    "),
            "    _detail_progress('base-program-translated')",
            ...
            "    _opts.SuppressAllInput = True",
            f"    _saved = Rhino.RhinoDoc.ActiveDoc.WriteFile({escaped['model']!r}, _opts)",
            "    _detail_progress('model-write-returned')",
            "    _status = {'schema': 'P069RhinoDetailCandidateRunStatus@1', "
            "'status': 'ok' if _saved else 'save_failed', "
            "'save_ok': bool(_saved), 'model_unit_system': 'Meters', "
            f"'source_vertical_axis': {_PANTHEON_COORDINATE_TRANSFORM['source_vertical_axis']!r}, "
            ...
            "        json.dump(_status, _f, sort_keys=True, "
            "ensure_ascii=True)",
            "    _detail_progress_state['status'] = _status.get('status')",
            "    _detail_progress('status-written')",
            "    Rhino.RhinoApp.Exit(False)",
            "",
        )
    )
...
def _record_candidate_cad_execution(
    ...
    *,
    ...
    program = load_program_shim(program_path)
    verification = _candidate_cad_verification_summary(
        program,
        measures=measures,
        semantics=semantics,
    )
    datum_readback = _candidate_cad_datum_readback(measures)
    verification["datum_readback"] = datum_readback
    from archflow.adapters.cad_program import expected_object_bounds

    expected_bounds = expected_object_bounds(program)
    axis_witness_name = "column-shaft-r0-c0-object"
    if axis_witness_name not in expected_bounds:
        raise RuntimeError("Stage 3 program lacks the required axis witness")
    model_gate = _headless_three_dm_gate(
        output_paths["model"],
        status,
        expected_object_count=int(verification["instance_count"]),
        axis_witness={
            "name": axis_witness_name,
            "source_bbox": expected_bounds[axis_witness_name],
        },
    )
    artifacts = {}
    for key, path in {
    ...
    verified = bool(
        ...
        and datum_readback["passed"] is True
...
def _record_candidate_detail_cad_execution(
    ...
    *,
    ...
    datum_readback = _candidate_cad_datum_readback(measures)
    base_verification["datum_readback"] = datum_readback
    from archflow.adapters.cad_program import expected_object_bounds
    ...
    model_gate = _headless_three_dm_gate(
        ...
        named_vertical_witnesses={
            "bronze-door-left": DOOR_THRESHOLD_Y,
            "bronze-door-right": DOOR_THRESHOLD_Y,
        },
    )
    detail_issues = []
    expected = {
    ...
    if detail.get("primary_inscription_codepoints") != [
        ord(character) for character in DETAIL_INSCRIPTION
    ]:
        detail_issues.append("primary inscription codepoints diverged")
    if detail.get("primary_inscription_geometry_mode") != (
        "rhino-text-object-no-relief"
    ):
        detail_issues.append("primary inscription geometry mode diverged")
    if int(detail.get("primary_inscription_text_object_count", 0)) != 1:
        detail_issues.append("primary inscription text object count diverged")
    if int(detail.get("primary_inscription_relief_count", -1)) != 0:
        detail_issues.append("parked inscription relief unexpectedly returned")
