"""P065: monument-scale staged derivation through the formal runtime.
...
from tests.integration.test_progressive_pantheon_proof import (
    IDENTITY,
    _canonical,
    _operation,
    _parameter,
...
def _cylinder(op_id, output, binding, y0, y1, radius):
    return _operation(
        op_id,
...
def _next_state(
    state: DevelopedDesignState, *, stage: int
    ...
    for component_id, maturity in _STAGE_REVISED[stage].items():
        ...
        remaining = tuple(
            ...
            if role
            not in {
                spec[0] for spec in _STAGE_COMPONENTS[stage]
            }
        )
    ...
    for component_id, parent, kind, intent in _STAGE_COMPONENTS[stage]:
...
class MonumentDerivationTests(unittest.TestCase):
    def test_four_stage_monument_reaches_instance_and_voxel_scale(self):
        ...
        self.assertEqual(
            [0, 1, 2, 3], [item["stage"] for item in manifest["stages"]]
        )
        scene = manifest["final_scene"]
        instances = _instance_count(scene)
        ...
        components = {
            item.component_id
            for item in (
                state.selected_schematic.option.proposal.components
            )
        }
        self.assertEqual(
            {
