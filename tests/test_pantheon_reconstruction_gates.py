...
class PantheonCandidateGeometrySelfCheckTests(unittest.TestCase):
        ...
        with patch(
            ...
            result = P._headless_three_dm_gate(
                P.Path("candidate.3dm"),
                {
                    ...
                    **{
                        field: P._PANTHEON_COORDINATE_TRANSFORM[field]
                        for field in (
    ...
    def test_detail_plan_freezes_massing_and_corrects_the_column_order_proxy(self):
        ...
        plan = P._detail_enrichment_plan(
            program,
            datum_authorization_ref="project://pantheon/datum-authority.json",
            front_step_conflict_ref="project://pantheon/step-conflict.json",
        )
        ...
        self.assertTrue(
            all(
                item["base_y"] == P.FLOOR_Y
                for item in plan["massing_locks"]["column_centres"]
            )
        )
        self.assertEqual(
            P.FLOOR_Y + P.DETAIL_TOTAL_COLUMN_H,
            plan["massing_locks"]["portico_entablature_base_y_m"],
        )
        ...
        self.assertEqual(
            ...
            plan["detail_decisions"][1]["implementation"]["geometry_mode"],
        )
        self.assertIsNone(
            plan["detail_decisions"][1]["implementation"]["relief_depth_m"]
        )
        door = plan["detail_decisions"][2]["implementation"]
        self.assertEqual(P.DOOR_THRESHOLD_Y, door["threshold_y_m"])
        self.assertEqual(
            door["threshold_y_m"] + door["opening_height_m"],
            door["head_y_m"],
        )
        ...
        step_decision = next(
            item
            for item in plan["detail_decisions"]
            if item["decision_id"] == "front-approach-steps-and-datum"
        )
        self.assertEqual("SOFT_CANDIDATE", step_decision["constraint_class"])
        self.assertEqual("unresolved", step_decision["historical_status"])
        self.assertEqual(
            [5, 7],
            step_decision["implementation"]["open_count_alternatives"],
        )
        self.assertFalse(step_decision["historical_fact_authority"])
        self.assertIn(
    ...
    def test_detail_rhino_overlay_is_offline_and_keeps_external_asset_hidden(self):
        ...
        self.assertNotIn(P.DETAIL_INSCRIPTION, script)
        self.assertIn("\\u00b7", script)
        self.assertIn("_detail_inscription = u'M\\xb7AGRIPPA", script)
        self.assertNotIn("rs.ExplodeText", script)
        self.assertIn("_detail_progress('column-blocks-built')", script)
        self.assertIn("_detail_progress('inscription-text-created')", script)
        self.assertIn("rs.AddBlock", script)
        self.assertIn("rs.InsertBlock", script)
        self.assertIn("_door_threshold_z", script)
        self.assertNotIn("(x0, 18.86, 1.00)", script)
    ...
    def test_stage1_columns_are_symmetric_eight_four_four_revolves(self):
        ...
        seven_step_metrics = {**metrics, "front_step_count": 7}
        self.assertNotIn(
            "stage1.front_steps_not_5",
            P._candidate_structure_issues(1, seven_step_metrics),
        )
        decision = P._candidate_soft_decisions(1, seven_step_metrics)[
            "front-approach-steps-and-datum"
        ]
        self.assertEqual("SOFT_CANDIDATE", decision["epistemic_status"])
        self.assertEqual("OPEN_HUMAN_REVIEW", decision["conflict_status"])
        self.assertEqual([5, 7], decision["open_count_alternatives"])
        self.assertEqual("DRIFT", decision["candidate_realization_status"])
        self.assertFalse(decision["hard_check"])
        self.assertFalse(decision["historical_fact_authority"])
    ...
    def test_transition_height_is_grade_to_top_not_wall_solid_thickness(self):
        ...
        with patch.object(P, "_CONTRACTS", P.stage_contracts(refs)):
            realized = P._realized_declaration_values(program, 0)
        self.assertEqual(
