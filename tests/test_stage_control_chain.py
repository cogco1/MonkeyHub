...
import archflow.control as control_api
...
def test_control_package_exports_generic_stage_chain_contracts() -> None:
    expected = {
        "ArtifactShaBinding",
        "ComponentFunctionBaselineSource",
        "ComponentFunctionContract",
    ...
    assert expected <= set(control_api.__all__)
    assert all(getattr(control_api, name, None) is not None for name in expected)
