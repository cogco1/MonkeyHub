...
@dataclass(frozen=True, slots=True)
class SemanticCompletenessReceipt:
    """Persistence-neutral proof that an exact denominator was assessed."""

    receipt_id: str
    denominator_id: str
    denominator_digest: str
    origin_branch: BranchRef
    origin_state_digest: str
    assessed_branch: BranchRef
    assessed_state_digest: str
    stage_id: str
    profile_id: str
    profile_digest: str
    profile_binding_id: str
    ...
    profile_requirement_id: str
    inherited_from_receipt_digest: str | None
    denominator_preserved: bool
    findings: tuple[SemanticSystemFinding, ...]
    reason_codes: tuple[str, ...]
    status: CheckStatus

    SCHEMA = "SemanticCompletenessReceipt@1"
    ...
    def __post_init__(self) -> None:
        ...
        if self.inherited_from_receipt_digest is not None:
            object.__setattr__(
                self,
                "inherited_from_receipt_digest",
                require_sha256(
                    self.inherited_from_receipt_digest,
                    "inherited_from_receipt_digest",
                ),
            )
        if not isinstance(self.origin_branch, BranchRef) or not isinstance(
            self.assessed_branch,
            BranchRef,
        ):
            raise TypeError("receipt branches must be BranchRef values")
        self.origin_branch.run.base.require_digest()
        self.assessed_branch.run.base.require_digest()
        origin_lineage = (
            self.origin_branch.run,
            self.origin_branch.branch_id,
        )
        assessed_lineage = (
            self.assessed_branch.run,
            self.assessed_branch.branch_id,
        )
        if origin_lineage != assessed_lineage:
            raise GenesisSemanticCompletenessError(
                "semantic completeness receipt crossed branch lineage"
            )
        if self.assessed_branch.epoch < self.origin_branch.epoch:
            raise GenesisSemanticCompletenessError(
                "semantic completeness receipt moved before genesis"
            )
        if type(self.denominator_preserved) is not bool:
            raise TypeError("denominator_preserved must be bool")
        if not isinstance(self.findings, tuple) or not self.findings or any(
            not isinstance(item, SemanticSystemFinding)
            for item in self.findings
        ):
            raise TypeError("findings must contain SemanticSystemFinding values")
        findings = tuple(sorted(self.findings, key=lambda item: item.system_ref))
        system_refs = tuple(item.system_ref for item in findings)
        if len(system_refs) != len(set(system_refs)):
            raise GenesisSemanticCompletenessError(
                "semantic completeness findings duplicate a system"
            )
        ...
        object.__setattr__(
            ...
            deterministic_identifiers(
                self.reason_codes,
                "semantic completeness reason_codes",
            ),
        )
        if not isinstance(self.status, CheckStatus):
            raise TypeError("status must be CheckStatus")
        expected = self.derived_status
        if self.status is not expected:
            raise GenesisSemanticCompletenessError(
                "semantic completeness status was caller-authored"
            )
        if self.inherited_from_receipt_digest is None:
            if (
                self.origin_branch != self.assessed_branch
                or self.origin_state_digest != self.assessed_state_digest
            ):
                raise GenesisSemanticCompletenessError(
                    "genesis receipt must assess its exact origin state"
                )
        elif self.assessed_branch.epoch <= self.origin_branch.epoch:
            raise GenesisSemanticCompletenessError(
                "inherited receipt must advance beyond its genesis epoch"
            )

    @property
    def system_refs(self) -> tuple[str, ...]:
        return tuple(item.system_ref for item in self.findings)

    @property
    def derived_status(self) -> CheckStatus:
        if not self.denominator_preserved or any(
            item.status is SemanticSystemFindingStatus.REJECTED
            for item in self.findings
        ):
            return CheckStatus.FAIL
        if any(
            item.status is SemanticSystemFindingStatus.OPEN
            for item in self.findings
        ):
    ...
    def _content_dict(self) -> dict[str, object]:
        return {
            ...
            "denominator_id": self.denominator_id,
            "denominator_digest": self.denominator_digest,
            "origin_branch": branch_ref_to_dict(self.origin_branch),
            "origin_state_digest": self.origin_state_digest,
            "assessed_branch": branch_ref_to_dict(self.assessed_branch),
            "assessed_state_digest": self.assessed_state_digest,
            "stage_id": self.stage_id,
            "profile_id": self.profile_id,
            "profile_digest": self.profile_digest,
            "profile_binding_id": self.profile_binding_id,
            ...
            "denominator_preserved": self.denominator_preserved,
    ...
    @classmethod
    def from_dict(cls, value: object) -> "SemanticCompletenessReceipt":
        payload = _exact_mapping(
            value,
            {
                "schema",
            ...
            "semantic completeness receipt",
        )
        if payload["schema"] != cls.SCHEMA:
            raise GenesisSemanticCompletenessError(
                "unsupported semantic completeness receipt schema"
            )
        _require_false_authority(payload)
        for field in ("findings", "reason_codes"):
            if not isinstance(payload[field], list):
                raise TypeError(f"{field} must be a list")
        result = cls(
            ...
            origin_branch=branch_ref_from_dict(payload["origin_branch"]),
            origin_state_digest=payload["origin_state_digest"],
            assessed_branch=branch_ref_from_dict(payload["assessed_branch"]),
            assessed_state_digest=payload["assessed_state_digest"],
            stage_id=payload["stage_id"],
            profile_id=payload["profile_id"],
            profile_digest=payload["profile_digest"],
            profile_binding_id=payload["profile_binding_id"],
            ...
            profile_requirement_id=payload["profile_requirement_id"],
            inherited_from_receipt_digest=payload[
                "inherited_from_receipt_digest"
            ],
            denominator_preserved=payload["denominator_preserved"],
            findings=tuple(
                SemanticSystemFinding.from_dict(item)
        ...
        if result.to_dict() != dict(payload):
            raise GenesisSemanticCompletenessError(
                "semantic completeness receipt digest changed"
            )
        return result
