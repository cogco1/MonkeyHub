...
## Stage evidence packs

`stage_evidence_pack.py` compiles an immutable audit index after branch
selection. A pack binds one exact project base, run, branch scope, decision
universe, basis/dependency/sufficiency records, design state, geometry program,
stage gate, convergence receipt, review, and content-addressed model artifact.
Stage N must cite the exact Stage N-1 pack and program digest; revisions append
through an explicit supersedes ref rather than overwriting.

`COMPLETE` means the cited materials were compiled with no blocking gap and
their closure summary is closed. It never means `ACCEPTED`: the pack has no
selection, evidence, stage-acceptance, or canonical-write authority. Missing
P078 scope, P079 sufficiency, P080 `stage_ready`, or a current model artifact
keeps compilation `INCOMPLETE` and remains visible to the progress panel.
