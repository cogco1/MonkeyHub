...
未缓存输入、输出和耗时可以分别排序；这些是定位调用的指标，不直接等于费用或自动优化结论。
...
| 几何导出 | 实际导出路径开始至完成、失败或取消 | 候选的内部明细，显示 OCCT/Rhino 与复用/增量/完整路径；不能再与候选总时长相加 |
...
模型调用耗时中位数仅使用明确的 `model_call` 计时。Token 数只在原始调用记录汇总一次，
任务分组、父子分组和操作关联不会创建第二份计数。旧日志仍可读，缺少新时间口径时显示未知。
运行中与最终操作使用同一事件 ID；异常或取消保留实际终止状态。监控写入失败不会重试模型、导出或保存。
浏览器仅在服务声明 `operation-timing` 时发送加载记录，关闭页面或断网可能缺失最后一条记录。
...
1. **先找大头。** 用当前采集器覆盖真实 Studio 请求和明确的开发会话；分别检查输入、缓存命中、输出、失败率及分段耗时。排队和正式发布目前没有独立计时。
