...
class MonitorData:
    ...
    def snapshot(self) -> dict:
        ...
        try:
            events.extend(replace(event, project_id=projects.get(event.session_id))
                          for event in iter_codex_events((*self.codex_sessions, *paths), warnings=warnings,
                                                         expected_sessions=paths))
        except (OSError, ValueError, UnicodeError):
