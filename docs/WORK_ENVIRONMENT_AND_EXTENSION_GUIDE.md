...
已有组合模型仍使用原 `modelSource`，直接投影整个组合模型，不借用某个局部 STEP。

3DM 原生读取支持平面 BRep（含孔洞）、直线挤出、支持的曲线及网格，并保留块变换、
单位与可见性。非平面 BRep 只有完整保存网格时才能作为近似几何出图；不支持的对象明确拒绝。
SKP 使用本机 SketchUp C API 独立读取，不打开 SketchUp：原 SKP 与派生 3DM 面网格
一起保留，再由同一投影/剖切路径出图。需要 Windows 上可用的 SketchUpAPI.dll；
缺少运行库时说明原因，不宣称已转换。该路径不还原 SKP 材质、原生线型、散线、注释或场景相机。
上述两条路径均无需手工转换 STEP，也不创建设计候选、Stage 或修改 HEAD。
