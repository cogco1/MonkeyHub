...
本切片不等于当前 #244 全部验收完成。人物/树木配景的后续补齐见下节；完整建筑图面的选择、表达与返改继续由 GitHub #244 跟踪，Issue 保持开放。


## SVG 配景补齐（#244）

同一 cut-plan 的 `viewRecipe.dressing` 现保存人物与树木平面符号，每个对象有独立 id、位置、尺寸、翻转和可选准确构件锚点。图纸界面直接拖动或用方向键移动，侧栏提供尺寸、翻转、删除和绑定；保存沿用原 Drawing revision/P036。SVG 的 `data-dressing` 组不带建筑 `data-object`，PNG 从同一 SVG 渲染。Board 仍负责评阅。

Agent 使用现有 `POST /api/drawings/plans`，带准确 `previousRevisionRef` 和 `dressingOperations`，批量执行 insert/move/scale/flip/delete；每项只改变指定配景，错误批次不会写入部分结果。`GET /api/drawings/plans/vector` 返回准确原图 SVG、内置符号和原来源构件锚点，用于可编辑预览，未新增项目状态或持久化所有者。

定位有两种明确含义：固定图面坐标，或相对于准确物理构件包围盒中心的偏移。显式重建时有效锚点跟随该构件位置；删除、不可用锚点保持原 id 并报告 missing，不吸附到其他构件。整个符号超出图框时报告 outside-view；这些未解析对象仍在 recipe 中，输出暂不绘制，用户可以重新定位或明确解除绑定。当前未承诺任意子拓扑锚点、旋转跟随、自动避让、外部任意 SVG 上传或符号语义升级。

真实房间测试覆盖独立多对象编辑、PNG/SVG 同源、冷重开、历史图保留、墙体移动后的跟随、删除断锚、非法批次原子拒绝，并核对 Design HEAD／已接受分支不变。实际 Drawing 组件浏览器测试覆盖拖动、键盘、缩放、翻转、删除、保存重开与窄屏；这些是工具流程验证，不代表用户已接受建筑方案或正式出图。

## 图面交付与界面收敛

主界面移除独立的“新增门宽尺寸”和设计门宽驱动流程。已存尺寸仍能读回、调整纸面偏移、提示断锚和删除；后端已有接口与历史数据不受影响。出图以当前保存版本为单位提供 SVG 下载，直接保留原 SVG 的毫米尺寸、比例、来源对象与配景。未保存的修改必须先保存，再下载；下载历史图不会混入当前新图的配景。

SVG 可继续交给 Illustrator 等矢量工具处理。本轮未实现任意外部编辑回传至模型、全套施工图能力或 Publish 的矢量 PDF 输出。
