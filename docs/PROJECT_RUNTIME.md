...
| Working Head and concurrent work | `GET /api/working-source` — the current working source each workspace follows; `GET /api/worktrees` — the read-only Worktree Graph of the head, running work, other lines and reconcile state | `studio.binding`, `studio.candidate` |
