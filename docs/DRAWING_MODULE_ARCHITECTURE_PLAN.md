...
**状态：** 2026-09-08 已明确与 MonkeyArch 平行的工作流边界，并将现有二维代码迁入 `monkeydiagram/` 及 Web 同名工作区；下文通用出图链仍为研究与实施建议，不表示全部能力已交付。
**审计基线：** 2026-09-04，`main`（`1e4d9cae`）及当时未提交的 Studio 工作树。
...
项目私有消费者已生成并登记的 SML 图纸；两个 Web 工作区已由现有切换入口装配。**仍需接入：** “打开旧图 →
