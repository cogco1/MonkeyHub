...
| GH-337 | active | One information hierarchy across the Hub (#337): plain text menus for navigation, parallel modes and commands; instrument UI only for what is being operated or must be acted on; quiet status words in a status line. Increment 1: the shared surface bar and status line, the Hub's top-left text menu row, and the Design Tree's chrome. | [GH-337](mapping/planning/GH-337-hierarchy.md) |
