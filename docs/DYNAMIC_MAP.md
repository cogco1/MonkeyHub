# ArchFlow V4 Dynamic Map

> Generated by `python tools/devctl.py render-map` from `governance/work_registry.json`.
> Architecture rationale is in [ARCHITECTURE.md](ARCHITECTURE.md); finished work is in Git history.

Phase: P115 is executing the authorized separation of ArchFlow shared core, MonkeyArch 3D and MonkeyDiagram drawing workflows in an isolated source worktree. The v0.1.0 source candidate is frozen. Existing feature and trial cards preserve their remaining acceptance criteria and resume after migration; no private project or active application is part of this change.

- active: P115
- ready: none
- blocked: P094, P105, P108, P110, P111, P113, P114
- next: continue active work: P115

## Live work

| ID | Status | Goal | Card |
| --- | --- | --- | --- |
| P115 | active | Migrate the existing shared ArchFlow core, MonkeyArch 3D workflow and MonkeyDiagram drawing workflow into explicit Python namespaces and peer Web workspaces, preserving behavior, retained project data and one P036 persistence authority. | [P115](mapping/planning/P115-capability-consolidation.md) |
| P094 | blocked | Prepare the CAADRIA 2027 full paper from the author-designated current Chinese abstract and retained records: Chinese working draft first, then an English manuscript in the applicable official template. The author selects the contribution and empirical scope; every quantitative claim resolves to its retained source. Paper work is currently paused. | [P094](mapping/planning/P094-caadria-2027-manuscript.md) |
| P105 | blocked | Give the canonical producers the vocabulary the monuments are written in: solids of revolution and the boolean chain the Pantheon's dome and drum need, and the Doric and Ionic member families plus the roof and entablature fields the Parthenon's 687 retained operations are made of, dimensioned by a derivation table in module ratios and placed by grid reference. Retires that vocabulary as operation kinds invented inside a per-building tool. | [P105](mapping/planning/P105-classical-order-producers.md) |
| P108 | blocked | Close the first usable edit-and-review round in the existing Studio web/API, preserving the landed viewer, launcher and kernel delegation. Verify a real project's intent, affected parts, candidate model and honestly scoped checks. The refoundation is not a new rebuild request; this round does not issue a project version. | [P108](mapping/planning/P108-vibe-modeling-frontend.md) |
| P110 | blocked | Connect actual retained obligations and authorized claims to the existing validation input so a violated project requirement cannot disappear behind empty facts. Follow the exact record references through one existing owner; do not treat a reference envelope as design content or invent facts to fill an old schema. Existing relation checks remain in use; candidate continuation need not wait for this entire card. | [P110](mapping/planning/P110-canonical-state-projection.md) |
| P111 | blocked | Follow up the existing-project trial of delivered candidate continuation: the selected run remains the next edit's exact base and preserves the prior revision. Implementation and isolated verification are complete; The remaining user trial is blocked pending the current page and a new specific edit. This does not establish whole-assembly usability or correct architectural dependencies. | [P111](mapping/planning/P111-continuing-design-cycle.md) |
| P113 | blocked | Make retained research evidence usable by one identified design or export consumer. First check existing Reading and source references; extend their existing owner if sufficient. The earlier ledger/import/export design is a proposal to revisit only if that consumer demonstrates a missing contract. | [P113](mapping/planning/P113-evidence-ledger.md) |
| P114 | blocked | User settings the launcher can read: GET/PUT /api/settings/user (language, theme, fontScale, model defaults) backed by %APPDATA%\MonkeyArch\settings.json in local mode only (remote mode neither declares the user-settings capability nor serves it), the launcher translating that file into environment variables at start, and the settings panel's save wired to the PUT with a user source badge. No remote multi-user. | [P114](mapping/planning/P114-user-settings-bridge.md) |
