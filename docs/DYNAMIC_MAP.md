...
- next: continue active work: GH-66, GH-294
...
| GH-294 | active | Implement candidate admission from the 2026-09-25 audit. S1 and S2: the CandidateAdmission@1 record in the fixed studio-admissions run and the gate it shares with Stage acceptance, the Candidate Pool readers for the #284 Design Tree and the retirement of Exploration creation. S3 and S4 (owner decision Q3, bounded automatic Agent authority): the Hub Agent closes each completed loop with one admission bound to the user's message, rejects or continues only on the user's bound words, and every Continue retains an attributed design.continued event. | [GH-294](mapping/planning/GH-294-admission-record.md) |
