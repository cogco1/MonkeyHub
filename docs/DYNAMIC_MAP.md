...
- next: continue active work: GH-307, GH-66
...
| GH-307 | active | Make a true section perspective: cut the retained STEP solids with a section plane, solve exact hidden lines with a perspective camera, fill the cut in poché, and retain it through the one drawing projection pipeline so the Agent, Board and Drawing read it like any other drawing. | [GH-307](mapping/planning/GH-307-section-perspective.md) |
