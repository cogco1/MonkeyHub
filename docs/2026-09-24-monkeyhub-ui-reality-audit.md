...
5. **Drawing 和 Render 的完整流程曾卡在“来源”这一步（第 9 节）。#273 合入后的情况：**
   - Render 已经可以把 Modeling 的当前视图抓取为冻结输入。
   - 门宽尺寸实验块和“创建设计候选”已移除。
   - Drawing 仍然只能画已经被接受为 Stage 的模型。
...
- **Q2（F1）已决定（2026-09-24，用户）：** 工作草稿的保存位置只由用户自己的 Sync 结果推进；agent 和提案生成的候选都不推进。这样刷新或重启之后，下一次编辑仍从用户的编辑基准开始。
  - **2026-09-25 再确认：** 用户在 Q2 与 #271/PR #277 的“工作头自动跟随”之间选择以 Q1/Q2 为准：
    - Drawing、Render、Board 自动跟随用户的编辑基准。
    - 用户自己的 Sync 推进这个基准。
    - agent 或提案生成的结果只是自动显示出来，要一键“从此版本继续”才会成为基准。
    - 与交互方案 §3.1 一致。
- **Q3（F10）已决定（2026-09-24，用户）：** 未被引用的候选不再自动永久删除，改为保留为备选，由用户显式拒绝或归档。已实现（lane `candidate-retention`）：
  - 清理不再删除任何 run，只让过期的崩溃恢复快照失效。
  - 候选列表不再限于最近 24 小时：这一改动在 lane `working-draft-position` 中，随 Q2 的前端修复一起合入。
  - 显式拒绝/归档的入口见 #289。
- **Q4（F6）：** 由交互方案给出建议：Hub 里只保留一个聊天，把当前选择作为上下文附带过去；还是保留两个，但赋予明确不同的角色？

## 9. Drawing、Render、Publish 验收

**2026-09-25 更新：** #273 已合入 main。对照 `b33c1b3e` 复核了以下三点（9.1–9.4 其余条目没有逐项复核）：
- **门宽尺寸：**
  - 固定的“门宽尺寸”实验块，以及其中的“修改设计宽度 → 创建设计候选”已经移除；`App.tsx` 里对应的 Drawing 请求流程也一并删除。
  - 侧栏改为“已有尺寸标注”列表，偏移量仍需手填纸面毫米。
  - 新建剖切图时不再带门宽实验（见浏览器场景 `drawingCanvas`）。
  - 在图面上直接点参照、拖放尺寸线，仍然没有实现。
- **Render：** `RenderWorkspace` 的 `captureModelView` 能把 Modeling 的透视或正交视图抓取为冻结输入（见浏览器场景 `renderWorkspace`）。“卡在选来源”这一步已经打通。
- **Drawing：** 仍然只能画已接受的 Stage（提示“请先在建模页面接受一个模型 Stage”）。

**核查对象：** main `04e71961`，以及 PR #270 head `05a2f314`（这个 PR 后来已关闭，未合并）。
