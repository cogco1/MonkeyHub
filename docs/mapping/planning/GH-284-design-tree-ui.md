...
- **Data.** The tree reads `GET /api/design-history`, `GET /api/working-source` and `GET /api/worktrees`. On the root task's instruction the lane merged `codex/294-admission-record` (2026-09-25) and reads the #294 `candidates[]`, `studies[]` and `warnings[]` and the Worktree Graph's admission verdicts through the generated client; the typed fixture serves tests and screenshots only. A runtime without `candidate-admission` shows Stages, Current and running work only.
