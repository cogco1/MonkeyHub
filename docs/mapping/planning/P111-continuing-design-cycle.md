...
**状态：** blocked（实现与隔离验证已交付；剩余试用在 MonkeyHub 的 Modeling 工作区进行（原 Studio 页面已退役），等待一项新的具体修改，不重放已完成请求）
...
这些局部整理项原由 [P115](P115-capability-consolidation.md) 跟踪，P115 已冻结为历史索引（#60）：C03（编译回执无消费者的自摘要）已交付；
C09（旧 developed／portfolio 投影适配）与 C10（确定性 proposal 的 provider 往返）是否仍需要未定，见 P115 卡的待决项。
它们不是本卡试用的强制前置。真实校核、程序身份和保留数据读取不因简化而删除。
