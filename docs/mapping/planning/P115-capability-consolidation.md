...
PR #62 已完成同版本原生构建、本机完整包和两个真实入口检查。按用户要求继续在本地 `codex/fab-in-hub` 合仓：将现有 Fab 移至 `apps/monkeyfab/`，统一依赖、CI 与单提交打包。原私有源码公开到 Hub 前仍需用户确认。用户随后要求桌面仅保留最新 App 入口；安装器随之只创建原生快捷方式并清理已识别的旧网页快捷方式。
