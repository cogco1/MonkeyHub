...
`P115/monitor-usage` 在独立 `codex/monitor-usage-fix` 分支 EXTEND `hub.shell` 与 `monkeymonitor`。
...
缺失来源与计数保持可见，日志没有单次请求起止时不补造 P50。74 项 Monitor 与 46 项 Hub 测试通过。
真实私有 Hub 自动拉起 Monitor 后，新增项目会话与归档归属无需手选或重启即可读取，重复快照不增量累计。
错误自动绑定与手选同路径时保留原手选读数，但不给未核实来源附加项目归属；联合解析继续处理继承扣除。
自动连接随包含本片的新 Hub 启动生效，当前活动服务仅恢复已有数值记录。
