...
| 效率主线 | #32，关联 #8 | ContextPack 切片见 PR #52；后续按同任务实测瓶颈选择优化，不先铺设广泛缓存或完整事务运行时 |
...
本轮进行中新增的 #54–#58 已纳入排期，不打断当前 #52/#53/#46 切片：

| 准备事项 | 相对顺序与首片边界 |
|---|---|
| #56 项目归档 | 这组中优先；在真实项目长期使用前，先做当前格式的导出→空目录恢复→正常读回，不等待通用迁移引擎 |
| #58 发行信任 | 与 #21 的公测包准备衔接；先核对 SECURITY、实际包内容/来源/依赖与验签路径，自动更新后置。仓库公开不等于安装包已正式发行 |
| #54 格式迁移 | 接 #56 恢复能力与 #21 真实旧数据；先检查已有版本字段，首个迁移必须对应真实旧格式变化，不为证明机制造 no-op 迁移或第二份格式权威 |
| #55 责任记录 | 在 #24 远程/多人写入扩大前；先核对现有 actor、操作、接受与发布记录能回答什么，再补一个真实缺口。复用 P036 留存，不复制 Monitor 或另建状态账本 |
| #57 扩展契约 | 等首个外部扩展的实际接入需求；复用已有专门接口。manifest 声明不等于隔离能力，host 必须真正控制其承诺拒绝的操作，Marketplace 后置 |
...
### #32：显式 ContextPack 切片

PR #52 在消息明确携带精确来源、基底与目标时，启动代理前准备只读上下文；
当前 UI 不自动提供该选择。取消会结束准备等待，停滞的绑定检查不阻碍 Hub 退出。
CLI 使用包含准备时间的总时限，ACP 保持原有可续期的无活动超时；准备本身有界且可取消。

同一请求的单次对照省去了初始 capability 查询，但代理随后额外读取 state：
工具调用仍为 3 次，总耗时 47.211→52.911 秒，首次可见 36.319→33.138 秒。
这不证明稳定提速或节费。完整请求可升级到 design tier 并返回整份记录，尚无上下文大小上限。
不为缩小数据省略 keep 条件，也不将此切片视为 #32 的整套事务与效率目标完成。
相关 Hub/真实 ACP SDK 回归 91 项通过；复核后的截止边界、取消、ACP 和进程退出检查 4 项通过。
该实现的来源和评审保留在 PR/Git，完成后不继续占用 live lane；下一片应由实际重复瓶颈决定。
...
用户要求不再为每次 UI 修复手动重装。沿 `hub.shell` 设置与原生生命周期、
`tools.package_monkeyapps` 打包和安装入口实现本地开发补丁：选择 ZIP 后校验当前版本，
只传送变化文件，复制未变文件，在同级目录拼出并核验完整新版；运行目录不被覆盖。
设置显示当前和目标版本、变化体积、准备状态及失败原因。

重启前拦截各会话未发送文字/附件、未同步模型、未保存设置、运行中的聊天和项目任务，
并关闭新写入的接收。已接收工作正常结束、旧 Hub 和桌面退出后，才启动新版试运行。
精确版本和自有进程健康核对、文件复核及既有安装入口切换成功后，才进入工作界面；
试运行失败先正常退出，再打开旧版并恢复入口。更新记录只在显式非项目 runtime/updates，
不迁移项目或聊天数据；这里的启动回退不表示任意后续项目格式都能向旧版降级。

验证包括真实文件增量重建与篡改拒绝、私有桌面目录下的入口激活、API 重启互斥及重开、
真实桌面/Hub 试运行、跨提交 helper 成功与启动/版本/激活失败回退，和设置页浏览器交互。
已有客户端需要一次装入更新器的完整版本，随后使用设置里的补丁入口。
当前为用户明确选择的未签名本地开发补丁；远程自动检查、签名发布源与无人值守下载仍未配置，
不以本片关闭 #58 的可信发布通道要求。包构建和安装状态另以实际交付结果报告。
...
PR #62 已完成同版本原生构建、本机完整包和两个真实入口检查。按用户要求继续在本地 `codex/fab-in-hub` 合仓：将现有 Fab 移至 `apps/monkeyfab/`，统一依赖、CI 与单提交打包。用户已明确批准将原私有 Fab 源码公开并合入 Hub，原仓库保留。用户随后要求桌面仅保留最新 App 入口；安装器随之只创建原生快捷方式并清理已识别的旧网页快捷方式。继续修复同一次项目投影反复构建依赖图、短暂健康失败引发的嵌入页连接错误，并统一中英文界面文案。
...
MonkeyFab 的算法在同仓 `apps/monkeyfab/` 中，Hub 继续调用其 CLI；它是制造准备模块，与建模、出图职责分开。
...
**本次结论。** 已有模型上的受支持修改、候选执行、保存与接续已形成实际调用链。把模糊指示发展为建筑方案，再检查、修正并交出完整成果，仍缺一段应用内的工作循环。本次先梳理现状；C08/C09 相关下一项建筑任务的范围，待作者确定。

**历史背景。** 作者补充：ARCH400 工作开展时，State 和 dependency graph 尚未完全做出来。项目脚本承担了当时缺失的建模和编排工作，并留下真实模型、图纸与可研究的方法。不能用今天的底座反推当时不该写脚本，也不能把这些成果归为无效工作。现在要判断的是：已有底座能够接管哪些反复发生的修改，哪些设计关系还留在每个项目的代码中。

### 整合前的三个代码基线（历史快照）

| 位置 | 本次核对时的状态 | 可以据此判断什么 |
| --- | --- | --- |
| 主线源码检出 | `main`，HEAD `3ca85aff`，含已有未提交的上下文与任务界面等修改 | 下表描述当前主线工作区的实际代码路径；不把已有 WIP 一并计作安装包能力 |
| 效率实验检出 | `codex/model-load-efficiency`，HEAD `3ffba934`，含本轮实验修改 | 已有输入精简、显式禁用工具的模型调用、限定请求的字段保持约束及相应验证；尚待集成 |
| 桌面快捷方式指向的 MonkeyHub | `versions/52b0cc25990f-fab-6128f99c8fbc`，candidate 安装包 | 比上述工作区旧；工作区中的改进不能直接算作桌面已交付 |

E 盘真模型复验验证了输入编译、回答与内存 operator 行为；另有模拟模型回答驱动的真实候选、持久化和重启测试。这两类证据尚未合成真实模型自主设计、检查、修复和 CAD 交付的整轮证明。详细记录保留在本机实验说明中，未随本仓库发布。
...
ARCH400 会话的课程项目保留在本机课程目录中，不在本仓库内。当前模型与图纸以其 `docs/CURRENT_STATE.md` 为准；本次读取的 E 平面、B 结构和组合生成脚本用于理解历史工作方式，不替代当前成果或用户的后续模型修改。
...
`5813fb4`；各实现会话从本次交接提交建立独立工作区，保留主线检出的未提交内容。
...
| W10 | 本机／远端 API 模式、token／CORS、协议能力声明、OpenAPI 生成 SDK、启动器与环境配置 | 保留；UI 设置持久化已交付，Hub 读取模型默认值并注入项目进程 | `studio.shell`、`hub.shell`、[PROTOCOL](../../PROTOCOL.md) |
...
| W12 | `labs/` 目前只有探索规则；比较实验和共享工具箱是独立工作线 | 保留分工，不把试验门槛加到普通建筑修改前 | [labs](../../../labs/README.md) |
...
| --- | --- | --- |
| 古典构件与旋转实体词汇 | [P105](P105-classical-order-producers.md) blocked，尚未开工 | 按真实缺失构件扩展 producer；当前已有楼梯／窗等，旧能力枚举需刷新；不复活逐建筑硬编码工具 |
| Studio 真实使用闭环 | [P108](P108-vibe-modeling-frontend.md) blocked；完整模型、图纸来源、A／B、批注恢复、界面收纳和新模型提示已交付；页面图像、完整批注与明确参照进入意图调用已在本地实现并通过隔离验收 | 等待可用的当前页面及新的具体修改意见，接入视觉输入并验证真实改稿效果；同一澄清保留首轮图像与修改起点，查看另一模型不改编辑来源 |
...
| 模型制造准备：3D 打印缩放拆件，后续板材排料与激光／CNC | P115 F14：MonkeyFab 独立工具实现闭合 STL／OBJ 等比缩放、X1C／H2S／H2D 封闭拆件与装配清单，以及已切片文件的 LAN Send；Hub 提供现有 CLI 的操作页面，完整包共用 Python 并绑定同一 Hub 提交 | 主归“出”，改变设计的拆件／接头回到“做”，工艺条件归“读”、制造检查归“验”；源码归入同仓 `apps/monkeyfab/`，Hub 保留固定工具入口，几何结果由用户明确指定外部目录。Stage 工件回写、实机发送验收、板材排料、激光／CNC 仍待实现 |
...
- [ ] **C11｜校准过时 P 卡文字（本轮相关部分完成）。** 已更新 P111 的 Program／options 来源限制，移除 P108 的旧重建清单和“尚无候选”快照，并将重复整理清单归到本卡。P108／P110／P111／P113 保留具体等待条件；用户设置已完成源码交接且由 Hub 接管启动读取，P114 卡与登记已退役。P105 的旧能力枚举仍待实际消费者核对；不关闭原功能验收，也不将尚未核对的卡标成完成。
...
**持续使用验收。** P108 保留完整模型与图纸批注连续使用的真实验收：完整资产显示、图纸到模型的明确来源、同一事项的 A／B 继续与重启恢复、3D 批注保存、后续候选保留外部设备。等待当前页面及新的具体修改意见后，沿同一入口试用并回填结果；不重做已交付的用户设置，也不同时启动其余研究、分析或制造计划。
