...
**Status:** implemented in the working tree; verification pending
...
exactness is the pair `change.base_record_digest == record.digest` and
`change.base_state_digest == record.state_digest`; it contains no propagation logic (propagation is
...
- [ ] A kernel function applies a typed intent to a `StateRecord@1`: exact-base check on the complete `record.digest`
      and its existing bound `state_digest`, protected
